"""
Pass map for SC Telstar: open-play passes starting in zone 14 (the central
channel just outside the box, x in [66.7, 83), y in [33, 67] on the Opta
100x100 grid) and their share landing in the shot hot zone — the box patch
identified in shot_hotspot_analysis.py as responsible for ~45% of the
league's shot xG from ~17% of the box's area (see shot_hot_zone.json).

Built in the Meridian house style (housestyle package).

Usage: python3 "Box Entry Models/telstar_zone14_to_hotzone_passmap.py" [out_dir]
"""
import sys
import re
import glob
...
def main():
    ...
    with open(Path(__file__).parent / "shot_hot_zone.json") as f:
        hot = json.load(f)["zone"]

    files = sorted(glob.glob(str(EVENTS_DIR / "*.json")))
    team_map = build_team_map(files)
    cid = team_map[TEAM_NAME]

    passes = load_zone14_passes(cid)
    targeted = [p for p in passes if in_zone(p["x1"], p["y1"], hot)]
    to_hot = [p for p in targeted if p["completed"]]

    print(f"{len(passes)} zone 14 pass attempts, {len(targeted)} aimed at the hot zone, "
          f"{len(to_hot)} of those completed "
          f"({len(to_hot) / max(len(targeted), 1) * 100:.1f}%).")
    ...
    pitch.polygon([[[hot["x_lo"], hot["y_lo"]], [hot["x_lo"], hot["y_hi"]],
                    [hot["x_hi"], hot["y_hi"]], [hot["x_hi"], hot["y_lo"]]]],
                  ax=pitch_ax, facecolor=palette["accent"], edgecolor=palette["accent"],
                  alpha=0.16, linewidth=1.6, zorder=1)
    pitch.text(hot["x_hi"] + 1.8, (hot["y_lo"] + hot["y_hi"]) / 2,
               "HOT ZONE", ax=pitch_ax, ha="center", va="center", fontsize=8.5,
    ...
    for p in targeted:
        pitch.lines(p["x0"], p["y0"], p["x1"], p["y1"], ax=pitch_ax,
                     ...
                     lw=2.0 if p["completed"] else 1.2,
                     alpha=0.8 if p["completed"] else 0.45,
                     zorder=3 if p["completed"] else 2, comet=p["completed"])

    pitch.scatter([p["x1"] for p in to_hot], [p["y1"] for p in to_hot], ax=pitch_ax,
                   s=36, color=ink_blue, edgecolors=palette["surface"],
    ...
    pitch.scatter([p["x1"] for p in incomplete], [p["y1"] for p in incomplete], ax=pitch_ax,
                   s=22, facecolor="none", edgecolors=palette["ink_muted"],
                   linewidths=1.0, zorder=3)

    top_player = None
    if targeted:
        from collections import Counter
        top_player = Counter(p["player"] for p in targeted).most_common(1)[0]

    components.header(
        fig,
        kicker="SC Telstar",
        title=f"Only {len(to_hot)} of {len(targeted)} zone 14 passes into the hot zone land",
        dek=f"Zone 14 passes aimed at the box's highest-value patch  ·  "
            f"{len(to_hot)}/{len(targeted)} completed "
            f"({len(to_hot) / max(len(targeted), 1) * 100:.0f}%)  ·  Eredivisie 2025/26",
        palette=palette,
    )
    ...
    out_path = out_dir / "telstar_zone14_to_hotzone_passmap.png"
