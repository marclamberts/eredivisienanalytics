...
def main():
    out_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
        OUT_DIR, "quantity_vs_quality_scatter.png")
    ...
    colors = [GOLD_RAMP(0.25 + 0.65 * (v / vmax)) for v in df["total_disruption_value_x1000"]]
    ax.scatter(df["actions_linked"], df["quality_x1000"], s=sizes, color=colors,
              edgecolors="#ffe9b8", linewidth=0.5, alpha=0.88, zorder=3)

    ax.axvline(x_med, color=PITCH_LINE, lw=1.1, zorder=1)
    ax.axhline(y_med, color=PITCH_LINE, lw=1.1, zorder=1)

    quad_kw = dict(fontsize=10, color=TEXT_SUB, fontweight="bold", alpha=0.85)
    ...
    for _, r in label_pool.iterrows():
        texts.append(ax.annotate(
            ...
            xytext=(6, 5), textcoords="offset points", fontsize=8.6, color="white", zorder=5))
    ...
    ax.set_xlabel("Linked defensive actions this season (volume)", fontsize=10.5, color=TEXT_SUB)
    ax.set_ylabel("Mean disruption value per action, x1000 (quality)", fontsize=10.5, color=TEXT_SUB)
    ax.tick_params(colors=TEXT_SUB, labelsize=9)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ax.set_xlim(left=-3)
    ...
    fig.text(0.02, 0.012, "Data via Opta | quality = value denied per action, not just how "
             "often a player makes a defensive action", fontsize=7.5, color=TEXT_FOOT)
