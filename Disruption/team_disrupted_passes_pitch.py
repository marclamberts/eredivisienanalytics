...
def main():
    ...
    out_name = args[1] if len(args) > 1 else f"disrupted_passes_{safe_name}.png"
    ...
    for theme in ("dark", "light"):
        ...
        for _, r in team_disr.iterrows():
            pitch.lines(r["linked_pass_start_x"], r["linked_pass_start_y"],
                       ...
                       color=ldv.GOLD, lw=1.6, alpha=0.4, zorder=2, comet=False)
            pitch.scatter(r["x"], r["y"], ax=ax, s=70, marker="X", color=ldv.GOLD,
                         edgecolors=ldv.BG, linewidth=0.8, alpha=0.85, zorder=3)

        n = len(team_disr)
        n_disruptors = team_disr["player_name"].nunique()
        n_teams = team_disr["team_name"].nunique()
        fig.text(0.5, 0.965, team_name, fontsize=27, fontweight="bold", ha="center", color=ldv.TEXT_MAIN)
        fig.text(0.5, 0.925,
        ...
        legend_elems = [
            Line2D([0], [0], color=ldv.GOLD, linewidth=2, alpha=0.6, label="Disrupted pass"),
            Line2D([0], [0], marker="X", color="none", markerfacecolor=ldv.GOLD, markeredgecolor=ldv.BG,
                  markersize=10, label="Point of disruption"),
        ]
        ax.legend(handles=legend_elems, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=2,
                 frameon=False, fontsize=9.5, labelcolor=ldv.LEGEND_TEXT)

        # side panel: who did it
        panel = fig.add_axes([0.71, 0.10, 0.27, 0.74])
        panel.set_facecolor(ldv.BG)
        panel.axis("off")
        panel.set_title("Who Disrupted Them", fontsize=15, fontweight="bold", color=ldv.TEXT_MAIN,
                        loc="left", pad=10)
        ...
        fig.text(0.02, 0.02, "Data via Opta | disruption value = P(pass completes) x xT it would "
                 ...
                 "denied", fontsize=7.6, color=ldv.TEXT_FOOT)
