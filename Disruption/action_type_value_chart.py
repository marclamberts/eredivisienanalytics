...
def main():
    out_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
        OUT_DIR, "action_type_value_chart.png")

    df = pd.read_csv(os.path.join(OUT_DIR, "all_eredivisie_disruption_values.csv"))
    ...
    colors = [GOLD_RAMP(0.3 + 0.6 * (v / vmax)) for v in g["mean_value_x1000"]]
    ...
    fig.text(0.02, 0.02, "Data via Opta | disruption value = P(pass completes) x xT it would "
             ...
             "threatening passes, not just any pass", fontsize=7.3, color=TEXT_FOOT)
