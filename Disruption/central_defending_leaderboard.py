...
from league_disruption_visuals import (compute_attack_directions, GOLD_RAMP, BG, PITCH_LINE,
                                       TEXT_SUB, TEXT_FOOT, add_logo)
...
def main():
    out_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
        OUT_DIR, "central_defending_leaderboard.png")
    ...
    matches = pd.read_csv(os.path.join(OUT_DIR, "disruption_value_team_summary.csv"))[
        ["team_name", "matches"]]
    ...
    colors = [GOLD_RAMP(0.3 + 0.6 * (v / vmax)) for v in g["per_match_x1000"]]
    ...
    ax.tick_params(axis="y", colors="white", labelsize=10.5, length=0)
