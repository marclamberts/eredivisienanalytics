...
from league_disruption_visuals import GOLD_RAMP, GOLD, BG, PITCH_LINE, TEXT_SUB, TEXT_FOOT, add_logo
...
def footer(fig, text):
    ...
    fig.text(0.02, 0.012, text, fontsize=7.3, color=TEXT_FOOT)
...
def viz_half_split(values, out_dir):
    ...
    axes[0].set_title("Total value denied", color="white", fontsize=12)
    axes[1].bar(["1st half", "2nd half"], means, color=colors, width=0.55, zorder=3)
    axes[1].set_title("Mean value per action", color="white", fontsize=12)
    for ax in axes:
        ...
        for spine in ("left", "bottom"):
            ax.spines[spine].set_color(PITCH_LINE)
        ax.grid(axis="y", color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ...
    fig.text(0.5, 0.925, "Eredivisie 2025/26 · Season · disruption value by half, league-wide",
             fontsize=10, ha="center", color=TEXT_SUB)
...
def viz_home_away(values, out_dir):
    ...
    y = np.arange(len(g))
    h = 0.36
    ax.barh(y + h / 2, g["home"], height=h, color=GOLD, label="Home", zorder=3)
    ax.barh(y - h / 2, g["away"], height=h, color=BLUE, label="Away", zorder=3)
    ax.set_yticks(y)
    ax.set_yticklabels(g["team_name"], fontsize=9.8, color="white")
    ...
    ax.legend(loc="lower right", frameon=False, fontsize=9.5, labelcolor="#c7ccd4")
    fig.text(0.5, 0.965, "Home vs. Away Disruption Value", fontsize=22, fontweight="bold", ha="center", color="white")
...
def viz_weekly_trend(values, out_dir):
    ...
    fig.patch.set_facecolor(BG)
    ax.set_facecolor(BG)
    ax.plot(weekly.index, weekly.values, color=GOLD, lw=2, marker="o", markersize=4, zorder=3)
    ax.fill_between(weekly.index, weekly.values, color=GOLD, alpha=0.12, zorder=2)
    ...
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(axis="y", color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ax.set_ylabel("Total disruption value, x1000", fontsize=10, color=TEXT_SUB)
    fig.text(0.5, 0.965, "Disruption Value Over the Season", fontsize=22, fontweight="bold", ha="center", color="white")
    fig.text(0.5, 0.93, "Eredivisie 2025/26 · league-wide total, weekly", fontsize=10.5, ha="center", color=TEXT_SUB)
...
def viz_difficulty_distribution(out_dir):
    import joblib
    ...
    fig.patch.set_facecolor(BG)
    ax.set_facecolor(BG)
    ax.hist(pass_df["difficulty"], bins=40, color=GOLD, alpha=0.85, zorder=3)
    ax.axvline(pass_df["difficulty"].median(), color="white", lw=1.2, linestyle="--", zorder=4)
    ax.text(pass_df["difficulty"].median() + 0.01, ax.get_ylim()[1] * 0.9,
           f"median {pass_df['difficulty'].median():.2f}", color="white", fontsize=9)
    ax.tick_params(colors=TEXT_SUB, labelsize=9)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(axis="y", color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ax.set_xlabel("Pass difficulty (1 - predicted completion probability)", fontsize=10, color=TEXT_SUB)
    ax.set_ylabel("Passes", fontsize=10, color=TEXT_SUB)
    fig.text(0.5, 0.965, "How Hard Is the Average Pass?", fontsize=22, fontweight="bold", ha="center", color="white")
...
# 5. Top 5 teams, multi-axis radar
def viz_team_radar(values, out_dir):
    team_matches = pd.read_csv(os.path.join(OUT_DIR, "disruption_value_team_summary.csv"))
    ...
    colors = [GOLD, BLUE, GREEN, RED, "#c084fc"]
    ...
    ax.grid(color=PITCH_LINE, alpha=0.6)
    ax.legend(loc="center left", bbox_to_anchor=(1.22, 0.5), frameon=False, fontsize=10, labelcolor="white")
    fig.text(0.5, 0.97, "Top 5 Teams: Defensive Profile", fontsize=18, fontweight="bold", ha="center", color="white")
    fig.text(0.5, 0.935, "Eredivisie 2025/26 · min-max normalized across all 18 teams", fontsize=9.5,
             ha="center", color=TEXT_SUB)
...
def viz_disruptor_vs_disrupted(values, out_dir):
    ...
    matches = pd.read_csv(os.path.join(OUT_DIR, "disruption_value_team_summary.csv")).set_index("team_name")["matches"]
    g = g.join(matches)
    ...
    fig.patch.set_facecolor(BG)
    ax.set_facecolor(BG)
    ax.scatter(g["conceded_pm"], g["dealt_pm"], s=180, color=GOLD, edgecolors="#ffe9b8",
    ...
    for _, r in g.iterrows():
        ...
        ax.annotate(r["team_name"], xy=(r["conceded_pm"], r["dealt_pm"]), xytext=xytext,
                   textcoords="offset points", fontsize=8.3, color="white",
                   ha="right" if near_right_edge else "left")
    xmed, ymed = g["conceded_pm"].median(), g["dealt_pm"].median()
    ax.axvline(xmed, color=PITCH_LINE, lw=1)
    ax.axhline(ymed, color=PITCH_LINE, lw=1)
    ax.text(ax.get_xlim()[1] * 0.98, ax.get_ylim()[1] * 0.98, "SOLID BOTH WAYS", ha="right", va="top",
           fontsize=9, color=TEXT_SUB, fontweight="bold")
    ax.text(ax.get_xlim()[0] * 1.0 + 0.02, ax.get_ylim()[1] * 0.98, "VULNERABLE, BUT DISRUPTS", ha="left",
           va="top", fontsize=9, color=TEXT_SUB, fontweight="bold")
    ax.margins(x=0.10, y=0.08)
    ax.set_xlabel("Value conceded per match (passes disrupted BY opponents)", fontsize=10, color=TEXT_SUB)
    ax.set_ylabel("Value dealt per match (passes disrupted OF opponents)", fontsize=10, color=TEXT_SUB)
    ax.tick_params(colors=TEXT_SUB, labelsize=9)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(color=PITCH_LINE, linewidth=0.4, alpha=0.4)
    fig.text(0.5, 0.965, "Disruptor vs. Disrupted", fontsize=22, fontweight="bold", ha="center", color="white")
...
def viz_by_third(values, out_dir):
    from league_disruption_visuals import compute_attack_directions
    directions = compute_attack_directions()
    dir_vals = np.array([
        directions.get((mf, cid, p), 1)
    ...
    ax.set_facecolor(BG)
    left = np.zeros(len(g))
    colors = {"Defensive third": BLUE, "Middle third": GOLD, "Attacking third": GREEN}
    for col in ["Defensive third", "Middle third", "Attacking third"]:
        ...
        left += g[col].values
    ax.tick_params(axis="y", colors="white", labelsize=9.8, length=0)
    ax.tick_params(axis="x", colors=TEXT_SUB, labelsize=9)
    for spine in ("top", "right", "left"):
        ax.spines[spine].set_visible(False)
    ax.spines["bottom"].set_color(PITCH_LINE)
...
def viz_value_boxplot(values, out_dir):
    ...
    bp = ax.boxplot(data, vert=True, patch_artist=True, showfliers=True,
                    flierprops=dict(marker="o", markersize=3, markerfacecolor=GOLD,
    ...
    ax.set_xticklabels(order, rotation=55, ha="right", fontsize=7.6, color="white")
    ax.tick_params(axis="y", colors=TEXT_SUB, labelsize=9)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(axis="y", color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ax.set_ylabel("Disruption value per action, x1000", fontsize=10, color=TEXT_SUB)
...
def viz_hall_of_fame(values, out_dir):
    ...
    colors = [GOLD_RAMP(0.3 + 0.6 * (v / vmax)) for v in top["value_x1000"]]
    ...
    for i, val in enumerate(top["value_x1000"]):
        ax.text(val + vmax * 0.012, i, f"{val:.2f}", va="center", fontsize=9.5, color="#c7ccd4")
    ax.tick_params(axis="x", colors=TEXT_SUB, labelsize=9)
    ax.tick_params(axis="y", length=0)
    for spine in ("top", "right", "left"):
        ax.spines[spine].set_visible(False)
...
def viz_cumulative_race(values, out_dir):
    ...
    colors = [GOLD, BLUE, GREEN, RED, "#c084fc", "#5eead4"]
    ...
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(PITCH_LINE)
    ax.grid(axis="y", color=PITCH_LINE, linewidth=0.5, alpha=0.4, zorder=0)
    ax.set_ylabel("Cumulative disruption value, x1000", fontsize=10, color=TEXT_SUB)
    ax.legend(loc="upper left", frameon=False, fontsize=9.5, labelcolor="white")
    fig.text(0.5, 0.965, "Season-Long Disruption Race", fontsize=22, fontweight="bold", ha="center", color="white")
...
def main():
    ...
    viz_difficulty_distribution(out_dir)
    viz_team_radar(values, out_dir)
    viz_disruptor_vs_disrupted(values, out_dir)
    viz_by_third(values, out_dir)
