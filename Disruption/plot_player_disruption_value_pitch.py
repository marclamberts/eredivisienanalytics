...
from league_disruption_visuals import compute_attack_directions, GOLD_RAMP, add_logo
...
def main():
    ...
    out_path = args[1] if len(args) > 1 else os.path.join(
        OUT_DIR, f"pitch_disruption_value_{safe_name}.png")
    ...
    colors = [GOLD_RAMP(0.3 + 0.65 * (v / vmax)) if v > 0 else "#3a3f4a"
             for v in player["disruption_value"]]
    edge_colors = [GOLD if v > 0 else "#5a6070" for v in player["disruption_value"]]
    ...
    legend_elems = [
        Line2D([0], [0], marker="o", color="none", markerfacecolor=GOLD, markeredgecolor=GOLD,
    ...
    fig.legend(handles=legend_elems, loc="lower center", bbox_to_anchor=(0.5, 0.815), ncol=2,
              frameon=False, fontsize=9.3, labelcolor="#c7ccd4")
    ...
    fig.text(0.5, 0.075, caption, fontsize=9.3, ha="center", color="#c7ccd4")
    ...
    fig.text(0.02, 0.02, "Data via Opta | disruption value = P(pass completes) x xT it would "
             "have added (Disruption/disruption_value_model.py)", fontsize=7.2, color=TEXT_FOOT)
