"""
Shared data loading + parsing for the FK Teplice vs FC Viktoria Plzeň
pre-match preview (Chance liga, CZ 2026-2027, matchday 3, 2026-08-08).
...
form/style base. As of today that is 2 matches each: Teplice won 3-1 at
home over Bohemians 1905, then won 1-0 away at Slovan Liberec; Plzeň lost
1-3 at home to Slovan Liberec, then drew 1-1 at home with Zbrojovka Brno.
...
Structurally this file is the same template as the sibling "Hradec
Kralove vs Ostrava" pre-match preview, re-pointed at a different fixture
-- the BOHEMIANS_ID/HRADEC_ID/TEPLICE_ID/PARDUBICE_ID/OSTRAVA_ID/
ZLIN_ID/ARTIS_BRNO_ID/MLADA_BOLESLAV_ID constants below are left
untouched (not renamed) because ALL_MATCHES further down still needs
them correct as 8 of the 16 teams in the league-wide season sample;
TEPLICE_ID doubles as this fixture's home team (it was already one of
those 8), and PLZEN_ID (this fixture's away team, also already one of
the 16 via ALL_MATCHES' literal Viktoria Plzeň entries) gets its own
constant here for the first time since no earlier report needed to name
it directly.
...
distance+angle geometric formula the earlier pre-match previews in this
repo used -- same fix as the Hradec Kralove vs Besiktas and Hradec
Kralove vs Ostrava post-match reports. The xT proxy (xt_value, further
down) is deliberately left on the old geometric model -- it's a smooth
location-value surface, not a real shot probability. compute_attack_
directions()'s per-half flip heuristic was checked against the same
failure mode found in the Besiktas report for both of this fixture's
teams' own matches (see the sanity check block at the bottom of this
file) and DID reproduce it here (e.g. Teplice's own shots in MD2 land
entirely on the wrong end of the pitch when the flip is applied), so --
same as the Besiktas fix -- the function is disabled and returns {}
unconditionally; see its own docstring for the detailed evidence.

Team IDs cross-checked two ways: (1) goal-scorer contestantId counts in
the Teplice-Bohemians feed against that match's 3-1 final score, and the
Liberec-Teplice feed against its 0-1 score, and (2) both fixture teams'
IDs also appear in "CZ 2026-2027 Matches.csv" as the fixture-3
...
FIXTURE_HOME_ID, FIXTURE_AWAY_ID = TEPLICE_ID, PLZEN_ID
FIXTURE_HOME_NAME, FIXTURE_AWAY_NAME = TEAM_NAMES[TEPLICE_ID], TEAM_NAMES[PLZEN_ID]
COMPETITION = "Chance Liga 2026/27, Matchday 3"
VENUE = "AGC Aréna Na Stínadlech, Teplice"
...
def compute_attack_directions(events):
    ...
    locations in the Hradec Kralove vs Besiktas report, and the same
    diagnostic run against this fixture's underlying matches reproduces the
    same failure here: e.g. Teplice's own shots in MD2 vs Slovan Liberec
    land entirely in the 0-30m range (all 16 of them), nowhere near the
    opponent's goal, when the flip is applied. This data feed appears to
    already self-orient each event to the acting team's own attacking
    direction (own goal at x=0, attacking toward x=100) regardless of
    period, so the manual flip is unnecessary and, near the 50 threshold,
    actively wrong. Returning {} makes norm_xy()'s directions.get(key, 1)
    default to no-flip for every event, which was verified to put every
    shot (both teams, both periods, both of this fixture's teams' matches)
    into a sensible attacking-third x range."""
    return {}
