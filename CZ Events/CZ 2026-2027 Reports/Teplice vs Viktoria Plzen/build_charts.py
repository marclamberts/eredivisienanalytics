"""
FK Teplice vs FC Viktoria Plzeň - Pre-Match Preview
=====================================================
Chance Liga 2026/27, matchday 3 (2026-08-08, 17:00 local, AGC Aréna Na
Stínadlech, Teplice). Fixture not yet played.

Same template as the sibling "Hradec Kralove vs Ostrava" pre-match
...
matchday snapshot:
  FK Teplice: 2 matches played, 2W 0D 0L (won 3-1 at home over
  Bohemians 1905, won 1-0 away at Slovan Liberec)
  FC Viktoria Plzeň: 2 matches played, 0W 1D 1L (lost 1-3 at home to
  Slovan Liberec, drew 1-1 at home with Zbrojovka Brno)
...
FIGSIZE = (13.33, 7.5)
TEP_C = CATEGORICAL_DARK[2]    # teal -- FK Teplice (home)
...
TEP_SHORT, PLZ_SHORT = "Teplice", "Plzeň"
...
def cover():
    ...
    fig.text(0.5, 0.64, md.FIXTURE_HOME_NAME.upper(), fontsize=34, fontweight="bold",
              color=TEP_C, family="serif", ha="center", va="center")
...
def fixture_context(tep, plz):
    ...
    facts = [
        ...
        ("Venue", f"{md.VENUE} (Teplice home)"),
        ("Kickoff", f"{md.MATCH_DATE}, {md.KICKOFF_LOCAL} local"),
        ("Round", "Both sides' 3rd league match of 2026/27"),
    ]
    y0 = 1.0
    ...
    col_x = [0.0, 0.52]
    for x, snap, name, color in zip(col_x, (tep, plz), (md.FIXTURE_HOME_NAME, md.FIXTURE_AWAY_NAME), (TEP_C, PLZ_C)):
        y = 0.58
...
def shot_maps_mw1(tep, plz):
    ...
    for ax, snap, color, name in ((ax1, tep, TEP_C, md.FIXTURE_HOME_NAME), (ax2, plz, PLZ_C, md.FIXTURE_AWAY_NAME)):
        for s in snap.shots:
...
def xg_snapshot(tep, plz):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    metrics = [
        ("xG created", tep.xg_for, plz.xg_for),
        ("xG conceded", tep.xg_against, plz.xg_against),
        ("Shots", len(tep.own(tep.shots)), len(plz.own(plz.shots))),
        ("Big chances", sum(1 for s in tep.own(tep.shots) if s["big_chance"]),
    ...
    for y, (label, b, h) in zip(ypos, metrics):
        ax.barh(y + 0.18, b, height=0.32, color=TEP_C)
    ...
    legend_elems = [Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=TEP_C,
    ...
    components.header(fig, kicker="Chance Quality",
                       title="Teplice have created more, and given up much less, so far this season",
                       dek=f"Season-to-date shot numbers, trained xG model  ·  {tep.matches_played} matches "
                           f"for Teplice, {plz.matches_played} for Plzeň",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def possession_thirds(tep, plz):
    ...
    for i, (snap, name, color) in enumerate(((tep, md.FIXTURE_HOME_NAME, TEP_C), (plz, md.FIXTURE_AWAY_NAME, PLZ_C))):
...
def progression_bars(tep, plz):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    bp = tep.own(tep.passes)
...
def duels_discipline(tep, plz):
    ...
    rows = []
    for kind in kinds:
        b = [d for d in tep.own(tep.duels) if d["action"] == kind]
    ...
    for y, (kind, b_n, b_won, h_n, h_won) in zip(ypos, rows):
        ...
        ax1.barh(y + 0.18, b_won, height=0.32, color=TEP_C)
    ...
    b_fouls, b_yellow, b_red = fouls_cards(tep)
    ...
    for y, (label, b, h) in zip(ypos2, disc_metrics):
        ax2.barh(y + 0.18, b, height=0.32, color=TEP_C)
...
def team_radar(tep, plz):
    fig, palette = new_fig()
    ax = fig.add_axes([0.26, 0.18, 0.48, 0.58], polar=True)

    bp = tep.own(tep.passes)
    hp = plz.own(plz.passes)
    b_touch = sum(1 for t in tep.own(tep.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in plz.own(plz.touches) if t["x"] >= 70)

    metrics = [
        ("xG created", tep.xg_for, plz.xg_for),
        ("xG conceded (inv.)", 1 / max(tep.xg_against, 0.05), 1 / max(plz.xg_against, 0.05)),
        ("Shots", len(tep.own(tep.shots)), len(plz.own(plz.shots))),
        ("Progressive passes", sum(1 for p in bp if p["progressive"]), sum(1 for p in hp if p["progressive"])),
        ...
        ("Pressing (inv. PPDA)", 1 / tep.ppda_for(), 1 / plz.ppda_for()),
    ]
    ...
    for vals, color, name in ((boh_norm, TEP_C, md.FIXTURE_HOME_NAME), (hkr_norm, PLZ_C, md.FIXTURE_AWAY_NAME)):
        pts = vals + [vals[0]]
...
def keys_to_the_game(tep, plz):
    palette, _ = style.apply("dark")
    fig = plt.figure(figsize=FIGSIZE)
    fig.patch.set_facecolor(palette["surface"])

    points = []
    if plz.xg_against < tep.xg_against:
        points.append(f"Plzeň have conceded less ({plz.xg_against:.2f} xG) than Teplice have "
                       f"({tep.xg_against:.2f} xG) across their {plz.matches_played} matches so far.")
    else:
        points.append(f"Teplice have conceded far less ({tep.xg_against:.2f} xG across {tep.matches_played} "
    ...
    if tep.ppda_for() < plz.ppda_for():
        points.append(f"Teplice have pressed higher so far (PPDA {tep.ppda_for():.1f} vs "
        ...
        points.append(f"Plzeň have pressed considerably higher so far (PPDA {plz.ppda_for():.1f} vs "
                       f"Teplice's {tep.ppda_for():.1f}) -- if that intensity travels on the road, Teplice's "
    ...
    if h_touch > b_touch:
        points.append(f"Plzeň have actually racked up more final-third touches ({h_touch} vs "
                       f"Teplice's {b_touch}) but converted them far less efficiently ({plz.xg_for:.2f} xG from "
                       f"{len(plz.own(plz.shots))} shots vs Teplice's {tep.xg_for:.2f} from "
                       f"{len(tep.own(tep.shots))}) -- territory alone hasn't been enough for them without "
                       "sharper shot selection.")
    else:
        points.append(f"Teplice have spent more of their matches in the final third ({b_touch} touches there "
                       f"vs Plzeň's {h_touch}) -- if that territorial edge holds at home, Plzeň will need "
                       "to defend deep for longer spells.")
    points.append(f"This reads as a genuine form gap, not a coin-flip: Teplice sit unbeaten so far "
                   f"({tep.record}, {tep.points} pts) with the far better underlying numbers, while Plzeň "
                   f"({plz.record}, {plz.points} pts) are still searching for their first win, having followed "
                   "their opening defeat with a home draw against Zbrojovka Brno -- the away trip to Teplice "
                   "is a tough spot to turn that around.")

    fig.text(0.5, 0.075, f"★ Small-sample caveat: every number above is drawn from {tep.matches_played} "
    ...
    components.header(fig, kicker="Keys To The Game",
                       title="Four things worth watching for on 2026-08-08",
...
def win_probability(tep, plz, sim):
    fig, palette = new_fig()
    ax1 = fig.add_axes([0.06, 0.34, 0.19, 0.32])
    ax2 = fig.add_axes([0.28, 0.34, 0.19, 0.32])
    _donut(ax1, sim["home_win"], TEP_C, palette, TEP_SHORT, "WIN PROBABILITY")
    ...
    ax3.set_title(f"Most likely scorelines ({TEP_SHORT}–{PLZ_SHORT})", color=palette["ink_primary"],
...
def report_card(tep, plz, sim):
    ...
    hp = plz.own(plz.passes)
    rows = [
        ("Season record", f"{tep.record} ({tep.points} pts)", f"{plz.record} ({plz.points} pts)"),
        ("xG created", f"{tep.xg_for:.2f}", f"{plz.xg_for:.2f}"),
        ("xG conceded", f"{tep.xg_against:.2f}", f"{plz.xg_against:.2f}"),
        ("Pass accuracy", f"{sum(1 for p in bp if p['completed']) / len(bp):.0%}",
         f"{sum(1 for p in hp if p['completed']) / len(hp):.0%}"),
        ("PPDA", f"{tep.ppda_for():.1f}", f"{plz.ppda_for():.1f}"),
        ("Projected win prob.", f"{sim['home_win']:.0%}", f"{sim['away_win']:.0%}"),
    ]

    ax = fig.add_axes([0.14, 0.20, 0.72, 0.55])
    ax.axis("off")
    ax.text(0.0, 1.0, md.FIXTURE_HOME_NAME, fontsize=12.5, fontweight="bold", color=TEP_C, ha="left", va="top")
...
def recoveries_by_third(tep, plz):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])
...
def turnovers_dangerous(tep, plz):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
...
def shot_zones_heatmap(tep, plz):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
    ...
    for ax, snap, color, name in ((ax1, tep, TEP_C, md.FIXTURE_HOME_NAME), (ax2, plz, PLZ_C, md.FIXTURE_AWAY_NAME)):
        ...
        cmap = "Greens" if color == TEP_C else "Blues"
        if xs:
...
def pace_vs_volume_ranking(league):
    ...
    for tid, tm in league.teams.items():
        ...
        is_boh = tid == md.TEPLICE_ID
        is_hkr = tid == md.PLZEN_ID
        color = TEP_C if is_boh else (PLZ_C if is_hkr else LEAGUE_MUTED)
        ...
        if is_boh:
            offset = (7, 9)  # Teplice has the highest pass volume of the 16 teams -- clear of neighbours
        elif is_hkr:
...
def verticality_ranking(league):
    ...
    for y, (tid, v) in zip(ypos, ranking):
        weight = "bold" if tid in (md.TEPLICE_ID, md.PLZEN_ID) else "normal"
...
def main():
    tep = md.TeamSnapshot(md.TEPLICE_ID)
    plz = md.TeamSnapshot(md.PLZEN_ID)
    sim = md.simulate_scorelines(tep.own(tep.shots), plz.own(plz.shots))
    league = md.LeagueSeason()

    cover()
    fixture_context(tep, plz)
    shot_maps_mw1(tep, plz)
    xg_snapshot(tep, plz)
    xg_flow_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "05", "teplice")
    xg_flow_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "06", "plzen")
    shot_quality_table_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "07", "teplice")
    shot_quality_table_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "08", "plzen")
    goal_buildups_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "09", "teplice")
    goal_buildups_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "10", "plzen")
    pass_network_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "11", "teplice")
    pass_network_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "12", "plzen")
    progressive_passes_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "13", "teplice")
    progressive_passes_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "14", "plzen")
    passing_directness_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "15", "teplice")
    passing_directness_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "16", "plzen")
    touch_heatmap_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "17", "teplice", "Greens")
    touch_heatmap_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "18", "plzen", "Blues")
    possession_thirds(tep, plz)
    progression_bars(tep, plz)
    field_tilt_over_time_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "21", "teplice")
    field_tilt_over_time_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "22", "plzen")
    ppda_pressing(tep, plz)
    defensive_actions(tep, plz)
    duels_discipline(tep, plz)
    recoveries_by_third(tep, plz)
    turnovers_dangerous(tep, plz)
    crossing_map_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "28", "teplice")
    crossing_map_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "29", "plzen")
    zone14_halfspace_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "30", "teplice")
    zone14_halfspace_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "31", "plzen")
    long_balls_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "32", "teplice")
    long_balls_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "33", "plzen")
    shot_assists_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "34", "teplice")
    shot_assists_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "35", "plzen")
    key_passes_leaderboard_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "36", "teplice")
    key_passes_leaderboard_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "37", "plzen")
    xt_flow_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "38", "teplice")
    xt_flow_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "39", "plzen")
    xt_leaderboard_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "40", "teplice")
    xt_leaderboard_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "41", "plzen")
    shot_zones_heatmap(tep, plz)
    key_players_team_page(tep, TEP_C, md.FIXTURE_HOME_NAME, "43", "teplice")
    key_players_team_page(plz, PLZ_C, md.FIXTURE_AWAY_NAME, "44", "plzen")
    team_radar(tep, plz)
    pace_vs_volume_ranking(league)
    verticality_ranking(league)
    keys_to_the_game(tep, plz)
    win_probability(tep, plz, sim)
    report_card(tep, plz, sim)
    print("Done.")
