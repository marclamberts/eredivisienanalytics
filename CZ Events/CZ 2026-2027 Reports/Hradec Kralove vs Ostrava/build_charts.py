"""
FC Hradec Králové vs FC Baník Ostrava - Pre-Match Preview
==========================================================
Chance Liga 2026/27, matchday 3 (2026-08-09, 17:00 local, FINEP Arena,
...
def xg_snapshot(boh, hkr):
    ...
    for y, (label, b, h) in zip(ypos, metrics):
        ...
        ax.barh(y - 0.18, h, height=0.32, color=OST_C)
    ...
    legend_elems = [Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=HKR_C,
                    ...
                    Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=OST_C,
...
def possession_thirds(boh, hkr):
    ...
    for i, (snap, name, color) in enumerate(((boh, md.FIXTURE_HOME_NAME, HKR_C), (hkr, md.FIXTURE_AWAY_NAME, OST_C))):
...
def duels_discipline(boh, hkr):
    ...
    for y, (kind, b_n, b_won, h_n, h_won) in zip(ypos, rows):
        ...
        ax1.barh(y - 0.18, h_won, height=0.32, color=OST_C)
    ...
    for y, (label, b, h) in zip(ypos2, disc_metrics):
        ...
        ax2.barh(y - 0.18, h, height=0.32, color=OST_C)
...
def team_radar(boh, hkr):
    ...
    for vals, color, name in ((boh_norm, HKR_C, md.FIXTURE_HOME_NAME), (hkr_norm, OST_C, md.FIXTURE_AWAY_NAME)):
        pts = vals + [vals[0]]
...
def win_probability(boh, hkr, sim):
    ...
    _donut(ax2, sim["away_win"], OST_C, palette, OST_SHORT, "WIN PROBABILITY")
    ...
    ax3.set_title(f"Most likely scorelines ({HKR_SHORT}–{OST_SHORT})", color=palette["ink_primary"],
...
def report_card(boh, hkr, sim):
    ...
    ax.text(1.0, 1.0, md.FIXTURE_AWAY_NAME, fontsize=12.5, fontweight="bold", color=OST_C, ha="right", va="top")
    n = len(rows)
...
def pace_vs_volume_ranking(league):
    ...
    for tid, tm in league.teams.items():
        ...
        is_hkr = tid == md.OSTRAVA_ID
        color = HKR_C if is_boh else (OST_C if is_hkr else LEAGUE_MUTED)
...
def verticality_ranking(league):
    ...
    for y, (tid, v) in zip(ypos, ranking):
        weight = "bold" if tid in (md.HRADEC_ID, md.OSTRAVA_ID) else "normal"
...
def main():
    ...
    xg_snapshot(boh, hkr)
    xg_flow_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "05", "hradec")
    xg_flow_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "06", "ostrava")
    shot_quality_table_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "07", "hradec")
    shot_quality_table_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "08", "ostrava")
    goal_buildups_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "09", "hradec")
    goal_buildups_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "10", "ostrava")
    pass_network_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "11", "hradec")
    pass_network_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "12", "ostrava")
    progressive_passes_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "13", "hradec")
    progressive_passes_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "14", "ostrava")
    passing_directness_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "15", "hradec")
    passing_directness_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "16", "ostrava")
    touch_heatmap_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "17", "hradec", "Greens")
    touch_heatmap_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "18", "ostrava", "Blues")
    possession_thirds(boh, hkr)
    progression_bars(boh, hkr)
    field_tilt_over_time_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "21", "hradec")
    field_tilt_over_time_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "22", "ostrava")
    ...
    crossing_map_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "28", "hradec")
    crossing_map_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "29", "ostrava")
    zone14_halfspace_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "30", "hradec")
    zone14_halfspace_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "31", "ostrava")
    long_balls_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "32", "hradec")
    long_balls_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "33", "ostrava")
    shot_assists_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "34", "hradec")
    shot_assists_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "35", "ostrava")
    key_passes_leaderboard_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "36", "hradec")
    key_passes_leaderboard_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "37", "ostrava")
    xt_flow_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "38", "hradec")
    xt_flow_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "39", "ostrava")
    xt_leaderboard_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "40", "hradec")
    xt_leaderboard_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "41", "ostrava")
    shot_zones_heatmap(boh, hkr)
    key_players_team_page(boh, HKR_C, md.FIXTURE_HOME_NAME, "43", "hradec")
    key_players_team_page(hkr, OST_C, md.FIXTURE_AWAY_NAME, "44", "ostrava")
    team_radar(boh, hkr)
