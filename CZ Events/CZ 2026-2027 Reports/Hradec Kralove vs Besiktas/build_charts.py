"""
FC Hradec Králové vs Beşiktaş Jimnastik Kulübü - Post-Match Analysis
==========================================================
European qualifier, leg 1 of 2 (2026-08-06, FINEP Arena). Beşiktaş won
0-1 away (S. Kılıçsoy 79').

Same template as the sibling "Hradec Kralove vs Pardubice" post-match
report, re-pointed at this fixture, Meridian house style (dark mode) --
...
AWAY_C = CATEGORICAL_DARK[1]   # terracotta -- Beşiktaş Jimnastik Kulübü
...
def match_summary(shots, passes, touches):
    ...
    components.header(fig, kicker="Match Summary",
                       title=f"{md.HOME_NAME} 0-1 {md.AWAY_NAME}",
...
def xg_flow(shots):
    ...
    components.header(fig, kicker="xG Flow",
                       ...
                       dek=f"{md.HOME_NAME} 0-1 {md.AWAY_NAME}  ·  cumulative expected goals by minute",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def goal_buildups(events, directions, shots):
    ...
    components.header(fig, kicker="Goal Build-Ups",
                       ...
                       dek=f"{md.HOME_NAME} 0-1 {md.AWAY_NAME}  ·  possession chain leading to each goal",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def progression_bars(passes):
    ...
    components.header(fig, kicker="Progression",
                       title=f"{md.HOME_NAME} edged the progression numbers, but rarely turned it into danger",
...
def defensive_actions(defs):
    ...
    components.header(fig, kicker="Defending",
                       title="Beşiktaş defended at a much higher volume, and cleared more under pressure",
...
def momentum_timeline(shots, cards, subs):
    ...
    components.header(fig, kicker="Match Narrative",
                       ...
                       dek=f"{md.HOME_NAME} 0-1 {md.AWAY_NAME}  ·  key moments timeline",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def win_probability_situation(shots, sim):
    ...
    components.header(fig, kicker="Match Odds",
                       ...
                       dek=f"{sim['n']:,}-simulation Monte Carlo from shot xG  ·  actual result 0-1",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
