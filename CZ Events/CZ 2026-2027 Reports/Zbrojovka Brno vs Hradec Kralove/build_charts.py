"""
FC Zbrojovka Brno vs FC Hradec Králové - Pre-Match Preview
============================================================
Chance Liga 2026/27, matchday 4 (2026-08-16, 17:00 local, ShipEx Aréna).
Zbrojovka Brno are HOME; Hradec Králové are AWAY. Fixture not yet
played.

Cloned from the sibling "Hradec Kralove vs Viktoria Plzen" pre-match
preview and re-pointed at this fixture -- note the home/away swap
versus that template: there, Hradec were host; here, Hradec travel to
Brno, so every home/away-dependent label, venue phrase and home-advantage
framing has been flipped accordingly. Meridian house style (dark), and
since there is no shared event feed for a match that hasn't been played
yet (nor for matchday 4/5/6 generally), every page is built from each
team's SEASON-TO-DATE totals -- every match played so far, pooled
together -- rather than a single matchday snapshot:
  FC Zbrojovka Brno: 3 matches played, 1W 1D 1L, 4 pts (won 3-1 at home
  over Sparta Praha, drew 1-1 away at Viktoria Plzeň, lost 0-1 at home
  to Slovan Liberec) -- xG for/against 2.81/4.27, PPDA 19.33
  FC Hradec Králové: 3 matches played, 2W 1D 0L, 7 pts (won 2-1 at home
  over Pardubice, drew 0-0 away at Bohemians 1905, won 2-1 at home over
  Baník Ostrava) -- xG for/against 4.38/2.14, PPDA 15.94
-- as an early-season form/style snapshot, not a head-to-head (these two
sides have not met yet this season). Both samples are 3 matches deep
(matchdays 1-3 complete for the full 16-team league); every page is
scoped to what that can honestly support (team-level shape and volume,
not a settled multi-game trend), and is labelled as such. These numbers
are reproduced by running match_data.py directly -- see its __main__
block -- and every prose bullet below is computed from TeamSnapshot at
build time, never hand-typed, so it cannot drift out of sync with them.

Full 50-page scope, carried over unchanged from the template (see the
sibling Hradec Kralove vs Ostrava report for the earlier 19-to-50-page
expansion this was originally built from): xG flow, an xT flow +
leaderboard, and a pass-tempo (m/s) vs pass volume scatter, plus the
full per-team repertoire this repo's post-match reports establish (shot
log, goal build-ups, pass networks, progressive passes, passing
...
combined-team and 16-team league-sample pages (recoveries, turnovers,
shot zones, verticality ranking). The 16-team league sample pools every
matchday-1/2/3 event feed this repo has (LeagueSeason) -- season to
date, not a single round. ALL_MATCHES itself is untouched from the
template.
...
FIGSIZE = (13.33, 7.5)
ZBR_C = CATEGORICAL_DARK[0]    # ink blue -- FC Zbrojovka Brno (home) -- same index used for
                                # Zbrojovka's OPP_C in the sibling "Sparta Praha Matchday 1
                                # Analysis" report, reused here for consistency
HKR_C = CATEGORICAL_DARK[2]    # teal -- FC Hradec Králové (away) -- same index used for HKR_C
                                # in every other Hradec report in this repo
GOOD_C = STATUS_DARK["good"]
WARN_C = STATUS_DARK["warning"]
LEAGUE_MUTED = "#5A6672"      # league-context gray, distinct from house axis gray
ZBR_SHORT, HKR_SHORT = "Zbrojovka", "Hradec Kr."
...
def cover():
    ...
    fig.text(0.5, 0.64, md.FIXTURE_HOME_NAME.upper(), fontsize=34, fontweight="bold",
              color=ZBR_C, family="serif", ha="center", va="center")
...
def fixture_context(zbr, hkr):
    ...
    facts = [
        ...
        ("Venue", f"{md.VENUE} ({md.FIXTURE_HOME_NAME} home)"),
        ("Kickoff", f"{md.MATCH_DATE}, {md.KICKOFF_LOCAL} local"),
        ("Round", "Matchday 4 -- season-to-date totals below reflect each side's matches played so far"),
    ]
    y0 = 1.0
    ...
    col_x = [0.0, 0.52]
    for x, snap, name, color in zip(col_x, (zbr, hkr), (md.FIXTURE_HOME_NAME, md.FIXTURE_AWAY_NAME), (ZBR_C, HKR_C)):
        y = 0.58
...
def shot_maps_mw1(zbr, hkr):
    ...
    for ax, snap, color, name in ((ax1, zbr, ZBR_C, md.FIXTURE_HOME_NAME), (ax2, hkr, HKR_C, md.FIXTURE_AWAY_NAME)):
        for s in snap.shots:
...
def xg_snapshot(zbr, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    metrics = [
        ("xG created", zbr.xg_for, hkr.xg_for),
        ("xG conceded", zbr.xg_against, hkr.xg_against),
        ("Shots", len(zbr.own(zbr.shots)), len(hkr.own(hkr.shots))),
        ("Big chances", sum(1 for s in zbr.own(zbr.shots) if s["big_chance"]),
    ...
    for y, (label, b, h) in zip(ypos, metrics):
        ax.barh(y + 0.18, b, height=0.32, color=ZBR_C)
    ...
    legend_elems = [Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=ZBR_C,
    ...
    components.header(fig, kicker="Chance Quality",
                       title="Hradec have created more and leaked far less so far this season",
                       dek=f"Season-to-date shot numbers, trained xG model  ·  {zbr.matches_played} matches "
                           f"for Zbrojovka, {hkr.matches_played} for Hradec",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def possession_thirds(zbr, hkr):
    ...
    for i, (snap, name, color) in enumerate(((zbr, md.FIXTURE_HOME_NAME, ZBR_C), (hkr, md.FIXTURE_AWAY_NAME, HKR_C))):
...
def progression_bars(zbr, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    bp = zbr.own(zbr.passes)
...
def duels_discipline(zbr, hkr):
    ...
    rows = []
    for kind in kinds:
        b = [d for d in zbr.own(zbr.duels) if d["action"] == kind]
    ...
    for y, (kind, b_n, b_won, h_n, h_won) in zip(ypos, rows):
        ...
        ax1.barh(y + 0.18, b_won, height=0.32, color=ZBR_C)
    ...
    b_fouls, b_yellow, b_red = fouls_cards(zbr)
    ...
    for y, (label, b, h) in zip(ypos2, disc_metrics):
        ax2.barh(y + 0.18, b, height=0.32, color=ZBR_C)
...
def team_radar(zbr, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.26, 0.18, 0.48, 0.58], polar=True)

    bp = zbr.own(zbr.passes)
    hp = hkr.own(hkr.passes)
    b_touch = sum(1 for t in zbr.own(zbr.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in hkr.own(hkr.touches) if t["x"] >= 70)

    metrics = [
        ("xG created", zbr.xg_for, hkr.xg_for),
        ("xG conceded (inv.)", 1 / max(zbr.xg_against, 0.05), 1 / max(hkr.xg_against, 0.05)),
        ("Shots", len(zbr.own(zbr.shots)), len(hkr.own(hkr.shots))),
        ("Progressive passes", sum(1 for p in bp if p["progressive"]), sum(1 for p in hp if p["progressive"])),
        ...
        ("Pressing (inv. PPDA)", 1 / zbr.ppda_for(), 1 / hkr.ppda_for()),
    ]
    labels = [m[0] for m in metrics]
    n = len(labels)
    zbr_norm = [m[1] / max(m[1], m[2], 1e-9) for m in metrics]
    hkr_norm = [m[2] / max(m[1], m[2], 1e-9) for m in metrics]

    angles = [i / n * 2 * math.pi for i in range(n)] + [0]
    for vals, color, name in ((zbr_norm, ZBR_C, md.FIXTURE_HOME_NAME), (hkr_norm, HKR_C, md.FIXTURE_AWAY_NAME)):
        pts = vals + [vals[0]]
...
def keys_to_the_game(zbr, hkr):
    palette, _ = style.apply("dark")
    fig = plt.figure(figsize=FIGSIZE)
    fig.patch.set_facecolor(palette["surface"])

    zbr_conceded = sum(m["opp_score"] for m in zbr.matches)
    hkr_conceded = sum(m["opp_score"] for m in hkr.matches)

    points = []
    if hkr.xg_against < zbr.xg_against:
        points.append(f"Hradec have conceded far less ({hkr.xg_against:.2f} xG across {hkr.matches_played} "
                       f"matches) than Zbrojovka have ({zbr.xg_against:.2f} xG, {len(zbr.against(zbr.shots))} "
                       f"shots faced) -- on the underlying numbers Brno's defence is the clearer weakness of the "
                       f"two sides, even though they have actually conceded only {zbr_conceded} goals in that "
                       "time, fewer than the shot count alone would suggest.")
    else:
        points.append(f"Zbrojovka have conceded less ({zbr.xg_against:.2f} xG) than Hradec have "
                       f"({hkr.xg_against:.2f} xG) across their {zbr.matches_played} matches so far.")

    if abs(zbr.ppda_for() - hkr.ppda_for()) < 1.0:
        points.append(f"Pressing intensity is essentially a wash so far (PPDA {zbr.ppda_for():.1f} for Zbrojovka "
                       f"vs {hkr.ppda_for():.1f} for Hradec) -- neither side has shown a clear high-press "
                       "identity yet, so expect this to be decided elsewhere.")
    elif hkr.ppda_for() < zbr.ppda_for():
        points.append(f"Hradec have pressed higher so far on the road (PPDA {hkr.ppda_for():.1f} vs "
                       f"Zbrojovka's {zbr.ppda_for():.1f} at home) -- expect the visitors to try to disrupt "
                       "Brno's build-up early rather than sit off and invite pressure.")
    else:
        points.append(f"Zbrojovka have actually pressed higher so far (PPDA {zbr.ppda_for():.1f} vs "
                       f"Hradec's {hkr.ppda_for():.1f}) -- expect the hosts to try to disrupt Hradec's build-up "
                       "early rather than sit off.")

    b_touch = sum(1 for t in zbr.own(zbr.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in hkr.own(hkr.touches) if t["x"] >= 70)
    if hkr.xg_for > zbr.xg_for:
        points.append(f"Hradec have created more (xG {hkr.xg_for:.2f} from {len(hkr.own(hkr.shots))} shots) than "
                       f"Zbrojovka have at home (xG {zbr.xg_for:.2f} from {len(zbr.own(zbr.shots))}), and their "
                       f"final-third touch count ({h_touch}) is not far off Zbrojovka's ({b_touch}) despite "
                       "playing two of their three matches away from home -- the visiting attack looks the more "
                       "reliable of the two so far, home advantage notwithstanding.")
    else:
        points.append(f"Zbrojovka have spent more of their matches in the final third ({b_touch} touches there "
                       f"vs Hradec's {h_touch}) -- if that territorial edge holds at home, Hradec will need "
                       "to defend deep for longer spells.")

    points.append(f"Hradec arrive as the form side on paper -- {hkr.record}, {hkr.points} pts from "
                   f"{hkr.matches_played} matches, unbeaten, with the tighter defensive record of the two -- "
                   f"while Zbrojovka ({zbr.record}, {zbr.points} pts) host on home turf but have shipped more "
                   "shots and more xG than they have faced in every match so far. On the underlying numbers this "
                   "reads as Hradec's game to lose on the road, not a routine home banker for Brno.")

    fig.text(0.5, 0.075, f"★ Small-sample caveat: every number above is drawn from {zbr.matches_played} matches "
                          f"for Zbrojovka and {hkr.matches_played} for Hradec so far this season. Treat as an "
                          "early style signal, not settled form.",
    ...
    components.header(fig, kicker="Keys To The Game",
                       title=f"Four things worth watching for on {md.MATCH_DATE}",
...
def win_probability(zbr, hkr, sim):
    fig, palette = new_fig()
    ax1 = fig.add_axes([0.06, 0.34, 0.19, 0.32])
    ax2 = fig.add_axes([0.28, 0.34, 0.19, 0.32])
    _donut(ax1, sim["home_win"], ZBR_C, palette, ZBR_SHORT, "WIN PROBABILITY")
    ...
    ax3.set_title(f"Most likely scorelines ({ZBR_SHORT}–{HKR_SHORT})", color=palette["ink_primary"],
...
def report_card(zbr, hkr, sim):
    ...
    hp = hkr.own(hkr.passes)
    rows = [
        ("Season record", f"{zbr.record} ({zbr.points} pts)", f"{hkr.record} ({hkr.points} pts)"),
        ("xG created", f"{zbr.xg_for:.2f}", f"{hkr.xg_for:.2f}"),
        ("xG conceded", f"{zbr.xg_against:.2f}", f"{hkr.xg_against:.2f}"),
        ("Pass accuracy", f"{sum(1 for p in bp if p['completed']) / len(bp):.0%}",
         f"{sum(1 for p in hp if p['completed']) / len(hp):.0%}"),
        ("PPDA", f"{zbr.ppda_for():.1f}", f"{hkr.ppda_for():.1f}"),
        ("Projected win prob.", f"{sim['home_win']:.0%}", f"{sim['away_win']:.0%}"),
    ]

    ax = fig.add_axes([0.14, 0.20, 0.72, 0.55])
    ax.axis("off")
    ax.text(0.0, 1.0, md.FIXTURE_HOME_NAME, fontsize=12.5, fontweight="bold", color=ZBR_C, ha="left", va="top")
...
def recoveries_by_third(zbr, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])
    ...
    zbr_counts = by_third(zbr)
    hkr_counts = by_third(hkr)
    zone_labels = ["Defensive third", "Middle third", "Attacking third"]
    n = len(zone_labels)
    ypos = np.arange(n)[::-1]
    maxval = max(zbr_counts + hkr_counts) * 1.2 or 1
    for y, label, b, h in zip(ypos, zone_labels, zbr_counts, hkr_counts):
        ax.barh(y + 0.18, b, height=0.32, color=ZBR_C)
        ax.barh(y - 0.18, h, height=0.32, color=HKR_C)
        ax.text(b + maxval * 0.015, y + 0.18, str(b), va="center", fontsize=10, color=palette["ink_primary"])
        ax.text(h + maxval * 0.015, y - 0.18, str(h), va="center", fontsize=10, color=palette["ink_primary"])
...
def turnovers_dangerous(zbr, hkr):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
...
def shot_zones_heatmap(zbr, hkr):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
    ...
    for ax, snap, color, name in ((ax1, zbr, ZBR_C, md.FIXTURE_HOME_NAME), (ax2, hkr, HKR_C, md.FIXTURE_AWAY_NAME)):
        ...
        cmap = "Greens" if color == ZBR_C else "Blues"
        if xs:
...
def pace_vs_volume_ranking(league):
    ...
    for tid, tm in league.teams.items():
        ...
        is_zbr = tid == md.ZBROJOVKA_ID
        is_hkr = tid == md.HRADEC_ID
        color = ZBR_C if is_zbr else (HKR_C if is_hkr else LEAGUE_MUTED)
        ax.scatter([x], [y], s=170 if (is_zbr or is_hkr) else 60, color=color,
                   edgecolors=palette["ink_primary"] if (is_zbr or is_hkr) else "none", linewidth=1.6,
                   zorder=5 if (is_zbr or is_hkr) else 3)
        if is_zbr:
            offset = (7, 12)  # label offset up clears Zbrojovka's nearest neighbours in this sample
        elif is_hkr:
            offset = (7, 9)  # label offset clears Hradec's nearest neighbours in this sample
        ...
        ax.annotate(md.ALL_TEAM_NAMES[tid], xy=(x, y), xytext=offset, textcoords="offset points",
                    fontsize=9.5 if (is_zbr or is_hkr) else 8,
                    color=palette["ink_primary"] if (is_zbr or is_hkr) else palette["ink_muted"],
                    fontweight="bold" if (is_zbr or is_hkr) else "normal")
...
def verticality_ranking(league):
    ...
    for y, (tid, v) in zip(ypos, ranking):
        weight = "bold" if tid in (md.ZBROJOVKA_ID, md.HRADEC_ID) else "normal"
...
def main():
    zbr = md.TeamSnapshot(md.ZBROJOVKA_ID)
    hkr = md.TeamSnapshot(md.HRADEC_ID)
    sim = md.simulate_scorelines(zbr.own(zbr.shots), hkr.own(hkr.shots))
    league = md.LeagueSeason()

    cover()
    fixture_context(zbr, hkr)
    shot_maps_mw1(zbr, hkr)
    xg_snapshot(zbr, hkr)
    xg_flow_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "05", "zbrojovka")
    xg_flow_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "06", "hradec")
    shot_quality_table_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "07", "zbrojovka")
    shot_quality_table_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "08", "hradec")
    goal_buildups_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "09", "zbrojovka")
    goal_buildups_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "10", "hradec")
    pass_network_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "11", "zbrojovka")
    pass_network_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "12", "hradec")
    progressive_passes_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "13", "zbrojovka")
    progressive_passes_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "14", "hradec")
    passing_directness_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "15", "zbrojovka")
    passing_directness_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "16", "hradec")
    touch_heatmap_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "17", "zbrojovka", "Blues")
    touch_heatmap_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "18", "hradec", "Greens")
    possession_thirds(zbr, hkr)
    progression_bars(zbr, hkr)
    field_tilt_over_time_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "21", "zbrojovka")
    field_tilt_over_time_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "22", "hradec")
    ppda_pressing(zbr, hkr)
    defensive_actions(zbr, hkr)
    duels_discipline(zbr, hkr)
    recoveries_by_third(zbr, hkr)
    turnovers_dangerous(zbr, hkr)
    crossing_map_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "28", "zbrojovka")
    crossing_map_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "29", "hradec")
    zone14_halfspace_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "30", "zbrojovka")
    zone14_halfspace_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "31", "hradec")
    long_balls_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "32", "zbrojovka")
    long_balls_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "33", "hradec")
    shot_assists_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "34", "zbrojovka")
    shot_assists_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "35", "hradec")
    key_passes_leaderboard_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "36", "zbrojovka")
    key_passes_leaderboard_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "37", "hradec")
    xt_flow_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "38", "zbrojovka")
    xt_flow_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "39", "hradec")
    xt_leaderboard_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "40", "zbrojovka")
    xt_leaderboard_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "41", "hradec")
    shot_zones_heatmap(zbr, hkr)
    key_players_team_page(zbr, ZBR_C, md.FIXTURE_HOME_NAME, "43", "zbrojovka")
    key_players_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "44", "hradec")
    team_radar(zbr, hkr)
    pace_vs_volume_ranking(league)
    verticality_ranking(league)
    keys_to_the_game(zbr, hkr)
    win_probability(zbr, hkr, sim)
    report_card(zbr, hkr, sim)
    print("Done.")
