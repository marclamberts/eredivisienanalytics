"""
Shared data loading + parsing for the FC Zbrojovka Brno vs FC Hradec
Králové pre-match preview (Chance liga, CZ 2026-2027, matchday 4,
2026-08-16, ShipEx Aréna). Zbrojovka Brno are HOME this time; Hradec
Králové -- the client's own team -- are AWAY (the reverse of the
sibling "Hradec Kralove vs Viktoria Plzen" report, where Hradec were
home).

Unlike the post-match reports in this repo, there is no event feed for
THIS fixture -- it hasn't been played yet, and no event feed exists yet
for matchday 4/5/6 generally. Instead this pulls each team's
SEASON-TO-DATE totals -- every match they have actually played so far,
pooled together, not a single matchday-1 snapshot -- as the
...
file: Zbrojovka Brno are 1W 1D 1L, 4 pts (won 3-1 at home over Sparta
Praha, drew 1-1 away at Viktoria Plzeň, lost 0-1 at home to Slovan
Liberec; xG for/against 2.81/4.27, shots for/against 27/44, PPDA 19.33)
and Hradec Králové are 2W 1D 0L, 7 pts (won 2-1 at home over Pardubice,
drew 0-0 away at Bohemians 1905, won 2-1 at home over Baník Ostrava; xG
for/against 4.38/2.14, shots for/against 35/38, PPDA 15.94). These
numbers are reproduced by running this file directly (see the
`__main__` block at the bottom) -- do not hand-edit them elsewhere;
any prose derived from them should be computed from TeamSnapshot, not
hardcoded. Same Opta MA3 event feed conventions as the rest of this repo
...
"Hradec Kralove vs Viktoria Plzen" pre-match preview, re-pointed at a
different fixture -- the BOHEMIANS_ID/HRADEC_ID/TEPLICE_ID/PARDUBICE_ID/
OSTRAVA_ID/ZLIN_ID/ARTIS_BRNO_ID/MLADA_BOLESLAV_ID/PLZEN_ID/LIBEREC_ID/
ZBROJOVKA_ID constants below are left untouched (not renamed) because
ALL_MATCHES further down still needs them correct as all 11
non-inline-literal teams in the league-wide season sample; ZBROJOVKA_ID
and HRADEC_ID now double as this fixture's home and away teams
respectively.

ALL_MATCHES' matchday-3 section lists all 8 matchday-3 games (all
played and on file), unchanged from the template, so every team in the
16-team league sample pools exactly 3 matches. Nothing in ALL_MATCHES /
ALL_TEAM_NAMES / ALL_TEAM_MATCHES was altered for this fixture.
...
its own docstring) -- carried over unchanged from the template, which
re-verified it against this same matchday-1-3 dataset; every shot for
both fixture teams, across all 6 combined matches, lands in a sensible
attacking-third x range with the flip off. Do not re-enable it.
...
Bohemians-Hradec feed against its 0-0 score, and the Hradec-Ostrava feed
against its 2-1 score (plus the Zbrojovka-Sparta 3-1, Plzeň-Zbrojovka
1-1 and Zbrojovka-Liberec 0-1 feeds against their own final scores), and
(2) both fixture teams' IDs also appear in "CZ 2026-2027 Matches.csv" as
the matchday-4 contestants, which additionally supplies this fixture's
venue/date/kickoff (no matching event feed exists for that match yet,
since it is still days away of "today").
...
FIXTURE_HOME_ID, FIXTURE_AWAY_ID = ZBROJOVKA_ID, HRADEC_ID
FIXTURE_HOME_NAME, FIXTURE_AWAY_NAME = TEAM_NAMES[ZBROJOVKA_ID], TEAM_NAMES[HRADEC_ID]
COMPETITION = "Chance Liga 2026/27, Matchday 4"
VENUE = "ShipEx Aréna"
...
if __name__ == "__main__":
    tep = TeamSnapshot(ZBROJOVKA_ID)
    plz = TeamSnapshot(HRADEC_ID)
    for snap in (tep, plz):
