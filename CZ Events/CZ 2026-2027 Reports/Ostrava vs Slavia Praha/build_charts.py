"""
FC Baník Ostrava vs SK Slavia Praha - Pre-Match Preview
==========================================================
Chance Liga 2026/27, matchday 2 (2026-08-01, 17:00 local, Městský
stadion - Vítkovice Aréna, Ostrava). Fixture not yet played.

Same template as the sibling "Bohemians vs Hradec Kralove" pre-match
...
yet, every page is built from each team's OWN matchday-1 fixture --
  FC Baník Ostrava: won 1-0 away at FC Zlín
  SK Slavia Praha: won 5-1 at home over 1. FC Slovácko
...
flow, an xT flow + leaderboard, and a pass-tempo (m/s) vs pass volume
scatter, plus the full per-team repertoire this repo's post-match
...
FIGSIZE = (13.33, 7.5)
OST_C = CATEGORICAL_DARK[2]    # teal -- FC Baník Ostrava
SLA_C = CATEGORICAL_DARK[0]    # ink blue -- FC Hradec Kralove (same slot as the sibling post-match report)
GOOD_C = STATUS_DARK["good"]
WARN_C = STATUS_DARK["warning"]
LEAGUE_MUTED = "#5A6672"      # league-context gray, distinct from house axis gray
OST_SHORT, SLA_SHORT = "Ostrava", "Slavia"
...
def cover():
    ...
    fig.text(0.5, 0.64, md.FIXTURE_HOME_NAME.upper(), fontsize=34, fontweight="bold",
              color=OST_C, family="serif", ha="center", va="center")
    ...
    fig.text(0.5, 0.47, md.FIXTURE_AWAY_NAME.upper(), fontsize=34, fontweight="bold",
              color=SLA_C, family="serif", ha="center", va="center")
...
def fixture_context(boh, hkr):
    ...
    col_x = [0.0, 0.52]
    for x, snap, name, color in zip(col_x, (boh, hkr), (md.FIXTURE_HOME_NAME, md.FIXTURE_AWAY_NAME), (OST_C, SLA_C)):
        y = 0.58
...
def shot_maps_mw1(boh, hkr):
    ...
    for ax, snap, color, name in ((ax1, boh, OST_C, md.FIXTURE_HOME_NAME), (ax2, hkr, SLA_C, md.FIXTURE_AWAY_NAME)):
        for s in snap.shots:
...
def xg_snapshot(boh, hkr):
    ...
    for y, (label, b, h) in zip(ypos, metrics):
        ax.barh(y + 0.18, b, height=0.32, color=OST_C)
        ax.barh(y - 0.18, h, height=0.32, color=SLA_C)
    ...
    legend_elems = [Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=OST_C,
                            markersize=12, label=md.FIXTURE_HOME_NAME, linewidth=0),
                    Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=SLA_C,
    ...
    components.header(fig, kicker="Chance Quality",
                       title="Slavia created far more, and gave up far less, on opening day",
...
def possession_thirds(boh, hkr):
    ...
    for i, (snap, name, color) in enumerate(((boh, md.FIXTURE_HOME_NAME, OST_C), (hkr, md.FIXTURE_AWAY_NAME, SLA_C))):
...
def duels_discipline(boh, hkr):
    ...
    for y, (kind, b_n, b_won, h_n, h_won) in zip(ypos, rows):
        ...
        ax1.barh(y + 0.18, b_won, height=0.32, color=OST_C)
        ax1.barh(y - 0.18, h_n, height=0.32, color=palette["axis"])
        ax1.barh(y - 0.18, h_won, height=0.32, color=SLA_C)
    ...
    for y, (label, b, h) in zip(ypos2, disc_metrics):
        ax2.barh(y + 0.18, b, height=0.32, color=OST_C)
        ax2.barh(y - 0.18, h, height=0.32, color=SLA_C)
...
def team_radar(boh, hkr):
    ...
    for vals, color, name in ((boh_norm, OST_C, md.FIXTURE_HOME_NAME), (hkr_norm, SLA_C, md.FIXTURE_AWAY_NAME)):
        pts = vals + [vals[0]]
...
def keys_to_the_game(boh, hkr):
    ...
    if hkr.xg_against < boh.xg_against:
        points.append(f"Slavia conceded far less ({hkr.xg_against:.2f} xG) than Ostrava did "
                       f"({boh.xg_against:.2f} xG) -- Ostrava's back line will need a cleaner day than "
                       f"the one at Zlín, where they won 1-0 despite giving up almost as much chance "
                       "quality as they created.")
    else:
        points.append(f"Ostrava conceded less ({boh.xg_against:.2f} xG) than Slavia did "
                       f"({hkr.xg_against:.2f} xG) on opening day.")
    if boh.ppda_for() < hkr.ppda_for():
        points.append(f"Ostrava pressed higher on opening day (PPDA {boh.ppda_for():.1f} vs "
                       f"{hkr.ppda_for():.1f}) -- expect them to try to disrupt Slavia's build-up early "
                       f"rather than sit off.")
    else:
        points.append(f"Slavia pressed higher on opening day (PPDA {hkr.ppda_for():.1f} vs "
                       f"{boh.ppda_for():.1f}) -- if that intensity travels, Ostrava's buildup play will "
                       f"be under pressure from the first whistle.")
    b_touch = sum(1 for t in boh.own(boh.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in hkr.own(hkr.touches) if t["x"] >= 70)
    if h_touch > b_touch:
        points.append(f"Slavia spent more of their opener in the final third ({h_touch} touches there vs "
                       f"Ostrava's {b_touch}) -- if that territorial edge travels away from home, Ostrava "
                       "will need to defend deep for longer spells.")
    else:
        points.append(f"Ostrava actually had more final-third touches in their opener ({b_touch} vs "
                       f"Slavia's {h_touch}) but converted them far less efficiently ({boh.xg_for:.2f} xG "
                       f"from {len(boh.own(boh.shots))} shots vs Slavia's {hkr.xg_for:.2f} from "
                       f"{len(hkr.own(hkr.shots))}) -- territory alone won't be enough without sharper "
                       "shot selection.")
    points.append(f"Both sides won their openers, so this is a genuine early-season form clash: Ostrava's "
                   f"tight 1-0 away at {md.TEAM_MW1[md.OSTRAVA_ID]['opponent_name']} against Slavia's "
                   f"5-1 demolition of {md.TEAM_MW1[md.SLAVIA_ID]['opponent_name']} at home -- a much "
                   "bigger margin of victory, even if Ostrava's underlying shot volume wasn't far off.")
    ...
    components.header(fig, kicker="Keys To The Game",
                       title="Four things worth watching for on 2026-08-01",
...
def win_probability(boh, hkr, sim):
    ...
    _donut(ax1, sim["home_win"], OST_C, palette, OST_SHORT, "WIN PROBABILITY")
    _donut(ax2, sim["away_win"], SLA_C, palette, SLA_SHORT, "WIN PROBABILITY")
    ...
    ax3.set_title(f"Most likely scorelines ({OST_SHORT}–{SLA_SHORT})", color=palette["ink_primary"],
...
def report_card(boh, hkr, sim):
    ...
    ax.axis("off")
    ax.text(0.0, 1.0, md.FIXTURE_HOME_NAME, fontsize=12.5, fontweight="bold", color=OST_C, ha="left", va="top")
    ax.text(1.0, 1.0, md.FIXTURE_AWAY_NAME, fontsize=12.5, fontweight="bold", color=SLA_C, ha="right", va="top")
    n = len(rows)
...
def shot_zones_heatmap(boh, hkr):
    ...
    for ax, snap, color, name in ((ax1, boh, OST_C, md.FIXTURE_HOME_NAME), (ax2, hkr, SLA_C, md.FIXTURE_AWAY_NAME)):
        ...
        cmap = "Greens" if color == OST_C else "Blues"
        if xs:
...
def pace_vs_volume_ranking(league):
    ...
    for tid, tm in league.teams.items():
        ...
        is_boh = tid == md.OSTRAVA_ID
        is_hkr = tid == md.SLAVIA_ID
        color = OST_C if is_boh else (SLA_C if is_hkr else LEAGUE_MUTED)
...
def verticality_ranking(league):
    ...
    for y, (tid, v) in zip(ypos, ranking):
        weight = "bold" if tid in (md.OSTRAVA_ID, md.SLAVIA_ID) else "normal"
...
def main():
    boh = md.TeamSnapshot(md.OSTRAVA_ID)
    hkr = md.TeamSnapshot(md.SLAVIA_ID)
    ...
    xg_snapshot(boh, hkr)
    xg_flow_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "05", "ostrava")
    xg_flow_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "06", "slavia")
    shot_quality_table_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "07", "ostrava")
    shot_quality_table_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "08", "slavia")
    goal_buildups_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "09", "ostrava")
    goal_buildups_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "10", "slavia")
    pass_network_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "11", "ostrava")
    pass_network_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "12", "slavia")
    progressive_passes_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "13", "ostrava")
    progressive_passes_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "14", "slavia")
    passing_directness_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "15", "ostrava")
    passing_directness_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "16", "slavia")
    touch_heatmap_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "17", "ostrava", "Greens")
    touch_heatmap_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "18", "slavia", "Blues")
    possession_thirds(boh, hkr)
    progression_bars(boh, hkr)
    field_tilt_over_time_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "21", "ostrava")
    field_tilt_over_time_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "22", "slavia")
    ...
    crossing_map_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "28", "ostrava")
    crossing_map_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "29", "slavia")
    zone14_halfspace_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "30", "ostrava")
    zone14_halfspace_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "31", "slavia")
    long_balls_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "32", "ostrava")
    long_balls_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "33", "slavia")
    shot_assists_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "34", "ostrava")
    shot_assists_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "35", "slavia")
    key_passes_leaderboard_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "36", "ostrava")
    key_passes_leaderboard_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "37", "slavia")
    xt_flow_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "38", "ostrava")
    xt_flow_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "39", "slavia")
    xt_leaderboard_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "40", "ostrava")
    xt_leaderboard_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "41", "slavia")
    shot_zones_heatmap(boh, hkr)
    key_players_team_page(boh, OST_C, md.FIXTURE_HOME_NAME, "43", "ostrava")
    key_players_team_page(hkr, SLA_C, md.FIXTURE_AWAY_NAME, "44", "slavia")
    team_radar(boh, hkr)
