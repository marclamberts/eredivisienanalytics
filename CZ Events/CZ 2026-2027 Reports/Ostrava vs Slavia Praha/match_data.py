"""
Shared data loading + parsing for the FC Baník Ostrava vs SK Slavia Praha
pre-match preview (Chance liga, CZ 2026-2027, matchday 2, 2026-08-01).
...
own matchday-1 event feed (Ostrava's 1-0 win away at FC Zlín; Slavia's
5-1 win at home over 1. FC Slovácko) as the early-season form/style base,
...
this file is the same template as the sibling "Bohemians vs Hradec
Kralove" pre-match preview, re-pointed at a different fixture -- the
BOHEMIANS_ID/HRADEC_ID/TEPLICE_ID/PARDUBICE_ID constants below are left
untouched (not renamed) because ALL_MATCHES further down still needs them
correct as 4 of the 14 teams in the league-wide matchday-1 sample; this
fixture's own two teams get their own OSTRAVA_ID/SLAVIA_ID/ZLIN_ID/
SLOVACKO_ID constants instead.

Team IDs cross-checked two ways: (1) goal-scorer contestantId counts in
the Zlín-Ostrava feed against that match's 0-1 final score (the 1-goal
side is Ostrava, scorer V. Jurečka), and the Slavia-Slovácko feed against
its 5-1 score, and (2) both fixture teams' IDs also appear in "CZ
2026-2027 Matches.csv" as the fixture-2 contestants, which additionally
...
SLAVIA_ID = "8kpapuorr6hf0vosnovbreqqd"
ZLIN_ID = "aj1nbeiatqrs6e47mnhjidn15"
SLOVACKO_ID = "bp5x8iw8pstucx6s4iqht6xqf"

TEAM_NAMES = {
    OSTRAVA_ID: "FC Baník Ostrava",
    SLAVIA_ID: "SK Slavia Praha",
    ZLIN_ID: "FC Zlín",
    SLOVACKO_ID: "1. FC Slovácko",
}
TEAM_SHORT = {
    OSTRAVA_ID: "Ostrava",
    SLAVIA_ID: "Slavia",
    ZLIN_ID: "Zlín",
    SLOVACKO_ID: "Slovácko",
}

# This fixture (not yet played)
FIXTURE_HOME_ID, FIXTURE_AWAY_ID = OSTRAVA_ID, SLAVIA_ID
FIXTURE_HOME_NAME, FIXTURE_AWAY_NAME = TEAM_NAMES[OSTRAVA_ID], TEAM_NAMES[SLAVIA_ID]
...
TEAM_MW1 = {
    ...
    SLAVIA_ID: dict(
        path=os.path.join(EVENTS_DIR, "2026-07-26_SK Slavia Praha - 1. FC Slovácko.json"),
        opponent_id=SLOVACKO_ID, opponent_name="1. FC Slovácko", venue="Sinobo Stadium, Prague",
        was_home=True, result="Won 5-1 at home",
    ),
}
...
if __name__ == "__main__":
    boh = TeamSnapshot(OSTRAVA_ID)
    hkr = TeamSnapshot(SLAVIA_ID)
    for snap in (boh, hkr):
