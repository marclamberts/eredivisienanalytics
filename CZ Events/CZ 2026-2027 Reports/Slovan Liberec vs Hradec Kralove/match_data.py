"""
Shared data loading + parsing for the FC Slovan Liberec vs FC Hradec
Králové pre-match preview (Chance liga, CZ 2026-2027, matchday 6,
2026-08-29).

Unlike the post-match reports in this repo, there is no event feed for
THIS fixture -- it hasn't been played yet, and no matchday-4/5/6 event
feeds exist for any team yet either. Instead this pulls each team's
...
file: Slovan Liberec is 2W 0D 1L, 6 points (won 3-1 away at Viktoria
Plzeň, lost 0-1 at home to Teplice, then won 1-0 away at Zbrojovka Brno
in their matchday-3 fixture) and Hradec Králové is 2W 1D 0L, 7 points
(won 2-1 at home over Pardubice, drew 0-0 away at Bohemians 1905, won
2-1 at home over Baník Ostrava). Unlike the sibling "Hradec Kralove vs
Viktoria Plzen" report
(where Hradec played at home), Hradec Králové is the AWAY side in THIS
fixture -- Slovan Liberec host at Stadion u Nisy. Same Opta MA3 event
feed conventions as the rest of this repo (see
Disruption/build_disruption_model.py, and the post-match report's own
match_data.py in the sibling "Hradec Kralove vs Pardubice" folder,
...
non-inline-literal teams in the league-wide season sample; LIBEREC_ID
and HRADEC_ID now double as this fixture's home and away teams.

ALL_MATCHES is reused UNCHANGED from the template -- matchdays 1-3, all
16 teams, all 8 matchday-3 games -- since no matchday 4/5/6 feeds exist
yet for this fixture or any other.
...
compute_attack_directions()'s per-half flip heuristic remains disabled
(see its own docstring, unchanged from the template) -- returning {}
makes norm_xy() default to no-flip for every event; this was verified
broken in earlier reports and must not be re-enabled here.
...
against its 2-1 score (Hradec Králové's own three matches), and (2)
both fixture teams' IDs also appear in "CZ 2026-2027 Matches.csv" and
this repo's other Liberec/Hradec report folders, which additionally
supply this fixture's venue/date/kickoff (no matching event feed exists
for this specific matchday-6 fixture yet, since it is still days away
of "today").
...
FIXTURE_HOME_ID, FIXTURE_AWAY_ID = LIBEREC_ID, HRADEC_ID
FIXTURE_HOME_NAME, FIXTURE_AWAY_NAME = TEAM_NAMES[LIBEREC_ID], TEAM_NAMES[HRADEC_ID]
COMPETITION = "Chance Liga 2026/27, Matchday 6"
