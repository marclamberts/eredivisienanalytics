"""
FC Slovan Liberec vs FC Hradec Králové - Pre-Match Preview
============================================================
Chance Liga 2026/27, matchday 6 (2026-08-29, 17:00 local, Stadion u
Nisy, Liberec). Fixture not yet played.

Same template as the sibling "Hradec Kralove vs Viktoria Plzen"
pre-match preview, re-pointed at this fixture: Meridian house style
(dark), and since there is no shared event feed for a match that
hasn't been played yet (nor for any matchday-4/5/6 fixture), every
page is built from each team's SEASON-TO-DATE totals -- every match
played so far, pooled together -- rather than a single matchday
snapshot:
  FC Slovan Liberec: 3 matches played, 2W 0D 1L, 6 pts (won 3-1 away
  at Viktoria Plzeň, lost 0-1 at home to Teplice, won 1-0 away at
  Zbrojovka Brno). xG for/against 4.87/4.66 across 50 shots for, 33
  against; PPDA 17.37.
  FC Hradec Králové: 3 matches played, 2W 1D 0L, 7 pts (won 2-1 at
  home over Pardubice, drew 0-0 away at Bohemians 1905, won 2-1 at
  home over Baník Ostrava). xG for/against 4.38/2.14 across 35 shots
  for, 38 against; PPDA 15.94.
-- as an early-season form/style snapshot, not a head-to-head. Both
sides have played exactly 3 matches, so the samples are even; every
page is scoped to what that can honestly support (team-level shape
and volume, not a settled multi-game trend), and is labelled as such.
Unlike the sibling "Hradec Kralove vs Viktoria Plzen" report where
Hradec Králové played at home, Hradec Králové are the AWAY side here
-- Slovan Liberec host at Stadion u Nisy -- so every home/away framing
below is flipped relative to that sibling report.
...
league sample pools every matchday-1, matchday-2 and matchday-3 event
feed this repo has (LeagueSeason) -- season to date, not a single round.
...
FIGSIZE = (13.33, 7.5)
LIB_C = CATEGORICAL_DARK[2]    # teal -- FC Slovan Liberec (home)
HKR_C = CATEGORICAL_DARK[0]    # ink blue -- FC Hradec Králové (away)
GOOD_C = STATUS_DARK["good"]
WARN_C = STATUS_DARK["warning"]
LEAGUE_MUTED = "#5A6672"      # league-context gray, distinct from house axis gray
LIB_SHORT, HKR_SHORT = "Liberec", "Hradec Kr."
...
def cover():
    ...
    fig.text(0.5, 0.64, md.FIXTURE_HOME_NAME.upper(), fontsize=34, fontweight="bold",
              color=LIB_C, family="serif", ha="center", va="center")
...
def fixture_context(lib, hkr):
    ...
    facts = [
        ...
        ("Venue", f"{md.VENUE} (Slovan Liberec home)"),
        ("Kickoff", f"{md.MATCH_DATE}, {md.KICKOFF_LOCAL} local"),
        ("Round", "Matchday 6 -- season-to-date totals below reflect each side's matches played so far"),
    ]
    y0 = 1.0
    ...
    col_x = [0.0, 0.52]
    for x, snap, name, color in zip(col_x, (lib, hkr), (md.FIXTURE_HOME_NAME, md.FIXTURE_AWAY_NAME), (LIB_C, HKR_C)):
        y = 0.58
...
def shot_maps_mw1(lib, hkr):
    ...
    for ax, snap, color, name in ((ax1, lib, LIB_C, md.FIXTURE_HOME_NAME), (ax2, hkr, HKR_C, md.FIXTURE_AWAY_NAME)):
        for s in snap.shots:
...
def xg_snapshot(lib, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    metrics = [
        ("xG created", lib.xg_for, hkr.xg_for),
        ("xG conceded", lib.xg_against, hkr.xg_against),
        ("Shots", len(lib.own(lib.shots)), len(hkr.own(hkr.shots))),
        ("Big chances", sum(1 for s in lib.own(lib.shots) if s["big_chance"]),
    ...
    for y, (label, b, h) in zip(ypos, metrics):
        ax.barh(y + 0.18, b, height=0.32, color=LIB_C)
    ...
    legend_elems = [Line2D([0], [0], marker="s", color=palette["surface"], markerfacecolor=LIB_C,
    ...
    components.header(fig, kicker="Chance Quality",
                       title="Liberec have created more, but leak far more at the back",
                       dek=f"Season-to-date shot numbers, trained xG model  ·  {lib.matches_played} matches "
                           f"for Liberec, {hkr.matches_played} for Hradec",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def possession_thirds(lib, hkr):
    ...
    for i, (snap, name, color) in enumerate(((lib, md.FIXTURE_HOME_NAME, LIB_C), (hkr, md.FIXTURE_AWAY_NAME, HKR_C))):
...
def progression_bars(lib, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])

    bp = lib.own(lib.passes)
...
def duels_discipline(lib, hkr):
    ...
    rows = []
    for kind in kinds:
        b = [d for d in lib.own(lib.duels) if d["action"] == kind]
    ...
    for y, (kind, b_n, b_won, h_n, h_won) in zip(ypos, rows):
        ...
        ax1.barh(y + 0.18, b_won, height=0.32, color=LIB_C)
    ...
    b_fouls, b_yellow, b_red = fouls_cards(lib)
    ...
    for y, (label, b, h) in zip(ypos2, disc_metrics):
        ax2.barh(y + 0.18, b, height=0.32, color=LIB_C)
...
def team_radar(lib, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.26, 0.18, 0.48, 0.58], polar=True)

    bp = lib.own(lib.passes)
    hp = hkr.own(hkr.passes)
    b_touch = sum(1 for t in lib.own(lib.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in hkr.own(hkr.touches) if t["x"] >= 70)

    metrics = [
        ("xG created", lib.xg_for, hkr.xg_for),
        ("xG conceded (inv.)", 1 / max(lib.xg_against, 0.05), 1 / max(hkr.xg_against, 0.05)),
        ("Shots", len(lib.own(lib.shots)), len(hkr.own(hkr.shots))),
        ("Progressive passes", sum(1 for p in bp if p["progressive"]), sum(1 for p in hp if p["progressive"])),
        ...
        ("Pressing (inv. PPDA)", 1 / lib.ppda_for(), 1 / hkr.ppda_for()),
    ]
    ...
    for vals, color, name in ((boh_norm, LIB_C, md.FIXTURE_HOME_NAME), (hkr_norm, HKR_C, md.FIXTURE_AWAY_NAME)):
        pts = vals + [vals[0]]
...
def keys_to_the_game(lib, hkr):
    palette, _ = style.apply("dark")
    fig = plt.figure(figsize=FIGSIZE)
    fig.patch.set_facecolor(palette["surface"])

    points = []
    if hkr.xg_against < lib.xg_against:
        points.append(f"Hradec have conceded far less ({hkr.xg_against:.2f} xG across {hkr.matches_played} "
                       f"matches) than Liberec have ({lib.xg_against:.2f} xG) -- defensive solidity is the "
                       "clearest gap between the two sides so far, not raw attacking output.")
    else:
        points.append(f"Liberec have conceded less ({lib.xg_against:.2f} xG) than Hradec have "
                       f"({hkr.xg_against:.2f} xG) across their {lib.matches_played} matches so far.")
    if abs(lib.ppda_for() - hkr.ppda_for()) < 1.0:
        points.append(f"Pressing intensity is essentially a wash so far (PPDA {lib.ppda_for():.1f} for Liberec "
    ...
    elif lib.ppda_for() < hkr.ppda_for():
        points.append(f"Liberec have pressed higher so far (PPDA {lib.ppda_for():.1f} vs "
                       f"{hkr.ppda_for():.1f}) -- expect them to try to disrupt Hradec's build-up early "
                       f"rather than sit off.")
    else:
        points.append(f"Hradec have actually pressed higher so far (PPDA {hkr.ppda_for():.1f} vs "
                       f"Liberec's {lib.ppda_for():.1f}) -- a more compact, disruptive shape on the road that "
                       "could suit them if Liberec try to play through the middle.")
    b_touch = sum(1 for t in lib.own(lib.touches) if t["x"] >= 70)
    h_touch = sum(1 for t in hkr.own(hkr.touches) if t["x"] >= 70)
    if hkr.xg_for > lib.xg_for:
        points.append(f"Hradec have actually created more (xG {hkr.xg_for:.2f} from {len(hkr.own(hkr.shots))} "
                       f"shots) than Liberec have (xG {lib.xg_for:.2f} from {len(lib.own(lib.shots))}), and their "
                       f"final-third touch count ({h_touch}) is not far off Liberec's ({b_touch}) -- Hradec's "
                       "attacking numbers travel, not just their defensive ones.")
    else:
        points.append(f"Liberec have generated more shot volume ({len(lib.own(lib.shots))} shots, {b_touch} "
                       f"final-third touches) than Hradec ({len(hkr.own(hkr.shots))} shots, {h_touch} final-third "
                       "touches) -- if that territorial edge holds at home, Hradec will need to defend deep for "
                       "longer spells and rely on their sharper conversion of the chances they do get.")
    points.append(f"This reads as a contrast in styles rather than a mismatch: Hradec Králové travel unbeaten "
                   f"({hkr.record}, {hkr.points} pts, {hkr.matches_played} matches played) with by far the "
                   f"tighter defensive record, while Liberec's own attack ({lib.record}, {lib.points} pts) has "
                   f"produced more shots and more xG at the other end ({len(lib.own(lib.shots))} shots for "
                   f"{lib.xg_for:.2f} xG) -- this looks like a game where Liberec's extra attacking volume meets "
                   "Hradec's tighter shape, not a routine home banker.")

    fig.text(0.5, 0.075, f"★ Small-sample caveat: every number above is drawn from {lib.matches_played} matches "
                          f"for Liberec and {hkr.matches_played} for Hradec so far this season. Treat as an early "
    ...
    components.header(fig, kicker="Keys To The Game",
                       title="Four things worth watching for on 2026-08-29",
...
def win_probability(lib, hkr, sim):
    fig, palette = new_fig()
    ax1 = fig.add_axes([0.06, 0.34, 0.19, 0.32])
    ax2 = fig.add_axes([0.28, 0.34, 0.19, 0.32])
    _donut(ax1, sim["home_win"], LIB_C, palette, LIB_SHORT, "WIN PROBABILITY")
    ...
    ax3.set_title(f"Most likely scorelines ({LIB_SHORT}–{HKR_SHORT})", color=palette["ink_primary"],
...
def report_card(lib, hkr, sim):
    ...
    hp = hkr.own(hkr.passes)
    rows = [
        ("Season record", f"{lib.record} ({lib.points} pts)", f"{hkr.record} ({hkr.points} pts)"),
        ("xG created", f"{lib.xg_for:.2f}", f"{hkr.xg_for:.2f}"),
        ("xG conceded", f"{lib.xg_against:.2f}", f"{hkr.xg_against:.2f}"),
        ("Pass accuracy", f"{sum(1 for p in bp if p['completed']) / len(bp):.0%}",
         f"{sum(1 for p in hp if p['completed']) / len(hp):.0%}"),
        ("PPDA", f"{lib.ppda_for():.1f}", f"{hkr.ppda_for():.1f}"),
        ("Projected win prob.", f"{sim['home_win']:.0%}", f"{sim['away_win']:.0%}"),
    ]

    ax = fig.add_axes([0.14, 0.20, 0.72, 0.55])
    ax.axis("off")
    ax.text(0.0, 1.0, md.FIXTURE_HOME_NAME, fontsize=12.5, fontweight="bold", color=LIB_C, ha="left", va="top")
...
def recoveries_by_third(lib, hkr):
    fig, palette = new_fig()
    ax = fig.add_axes([0.24, 0.16, 0.66, 0.56])
...
def turnovers_dangerous(lib, hkr):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
...
def shot_zones_heatmap(lib, hkr):
    fig, palette = new_fig()
    pitch = new_pitch(palette)
    ax1 = fig.add_axes([0.02, 0.10, 0.47, 0.62])
    ax2 = fig.add_axes([0.51, 0.10, 0.47, 0.62])
    ...
    for ax, snap, color, name in ((ax1, lib, LIB_C, md.FIXTURE_HOME_NAME), (ax2, hkr, HKR_C, md.FIXTURE_AWAY_NAME)):
        ...
        cmap = "Greens" if color == LIB_C else "Blues"
        if xs:
...
def pace_vs_volume_ranking(league):
    ...
    for tid, tm in league.teams.items():
        ...
        is_lib = tid == md.LIBEREC_ID
        is_hkr = tid == md.HRADEC_ID
        color = LIB_C if is_lib else (HKR_C if is_hkr else LEAGUE_MUTED)
        ax.scatter([x], [y], s=170 if (is_lib or is_hkr) else 60, color=color,
                   edgecolors=palette["ink_primary"] if (is_lib or is_hkr) else "none", linewidth=1.6,
                   zorder=5 if (is_lib or is_hkr) else 3)
        if is_lib:
            offset = (7, 12)  # Liberec's point -- label offset up/right to clear nearby teams
        elif is_hkr:
            offset = (7, 9)  # Hradec's point -- label offset up/right to clear nearby teams
        ...
        ax.annotate(md.ALL_TEAM_NAMES[tid], xy=(x, y), xytext=offset, textcoords="offset points",
                    fontsize=9.5 if (is_lib or is_hkr) else 8,
                    color=palette["ink_primary"] if (is_lib or is_hkr) else palette["ink_muted"],
                    fontweight="bold" if (is_lib or is_hkr) else "normal")
...
def verticality_ranking(league):
    ...
    for y, (tid, v) in zip(ypos, ranking):
        weight = "bold" if tid in (md.LIBEREC_ID, md.HRADEC_ID) else "normal"
...
def main():
    lib = md.TeamSnapshot(md.LIBEREC_ID)
    hkr = md.TeamSnapshot(md.HRADEC_ID)
    sim = md.simulate_scorelines(lib.own(lib.shots), hkr.own(hkr.shots))
    league = md.LeagueSeason()

    cover()
    fixture_context(lib, hkr)
    shot_maps_mw1(lib, hkr)
    xg_snapshot(lib, hkr)
    xg_flow_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "05", "liberec")
    xg_flow_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "06", "hradec")
    shot_quality_table_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "07", "liberec")
    shot_quality_table_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "08", "hradec")
    goal_buildups_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "09", "liberec")
    goal_buildups_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "10", "hradec")
    pass_network_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "11", "liberec")
    pass_network_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "12", "hradec")
    progressive_passes_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "13", "liberec")
    progressive_passes_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "14", "hradec")
    passing_directness_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "15", "liberec")
    passing_directness_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "16", "hradec")
    touch_heatmap_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "17", "liberec", "Greens")
    touch_heatmap_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "18", "hradec", "Blues")
    possession_thirds(lib, hkr)
    progression_bars(lib, hkr)
    field_tilt_over_time_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "21", "liberec")
    field_tilt_over_time_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "22", "hradec")
    ppda_pressing(lib, hkr)
    defensive_actions(lib, hkr)
    duels_discipline(lib, hkr)
    recoveries_by_third(lib, hkr)
    turnovers_dangerous(lib, hkr)
    crossing_map_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "28", "liberec")
    crossing_map_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "29", "hradec")
    zone14_halfspace_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "30", "liberec")
    zone14_halfspace_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "31", "hradec")
    long_balls_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "32", "liberec")
    long_balls_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "33", "hradec")
    shot_assists_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "34", "liberec")
    shot_assists_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "35", "hradec")
    key_passes_leaderboard_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "36", "liberec")
    key_passes_leaderboard_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "37", "hradec")
    xt_flow_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "38", "liberec")
    xt_flow_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "39", "hradec")
    xt_leaderboard_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "40", "liberec")
    xt_leaderboard_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "41", "hradec")
    shot_zones_heatmap(lib, hkr)
    key_players_team_page(lib, LIB_C, md.FIXTURE_HOME_NAME, "43", "liberec")
    key_players_team_page(hkr, HKR_C, md.FIXTURE_AWAY_NAME, "44", "hradec")
    team_radar(lib, hkr)
    pace_vs_volume_ranking(league)
    verticality_ranking(league)
    keys_to_the_game(lib, hkr)
    win_probability(lib, hkr, sim)
    report_card(lib, hkr, sim)
    print("Done.")
