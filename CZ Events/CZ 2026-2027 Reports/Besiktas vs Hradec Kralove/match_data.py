"""
Shared data loading + parsing for the Beşiktaş Jimnastik Kulübü vs FC
Hradec Králové post-match report (European qualifier, leg 2 of 2,
2026-08-13). Beşiktaş won this leg 1-0 (V. Černý, 39') to complete a
2-0 aggregate win over the tie -- Hradec Králové are eliminated (leg 1,
2026-08-06 at Hradec, finished 0-1 to Beşiktaş; see the sibling "Hradec
Kralove vs Besiktas" report for that match).
...
build_goalkick_shot_model.py). Team IDs unchanged from leg 1 (this
session already established them): HOME_ID/AWAY_ID are simply swapped
here since Beşiktaş were the home side this time.

Same two fixes already established for the leg-1 report, carried over
and re-verified against this match's own data rather than assumed:

1. Shots (build_shots' xg field) are scored with the repo's real trained
   model (Model/model_xg.pkl, via Model/xg_model.py), not the small ad
   hoc distance+angle geometric formula most other reports in this repo
   still use. The xT proxy (xt_value, further down) is deliberately left
   on the old geometric model -- a smooth "value of this location"
   surface, not a real shot probability.

2. compute_attack_directions()'s per-half flip is disabled for this
   report too (see its docstring below). Re-run for THIS match's own
   events: computing each team's average raw pass x per period and
   checking against the 50 threshold, then verifying every shot lands in
   the sensible attacking-third range with the flip off -- confirmed
   clean, so the same fix applies here on its own evidence, not by
   assumption from leg 1. Per the user's original "no just this report"
   instruction, this stays scoped to the two Beşiktaş-Hradec reports --
   the shared heuristic other report folders use is untouched.

Like leg 1, this feed's matchDetails carries no competition/tournament
name -- only leg=2, relatedMatchId, aggregateId and aggregateWinnerId,
confirming it's leg 2 of the same two-legged European tie. COMPETITION
below is therefore deliberately generic ("European Qualifier, Leg 2 of
2, Aggregate 2-0 Beşiktaş") rather than a guessed UEFA competition name.
VENUE is similarly left generic (Beşiktaş's home ground, Istanbul) since
the feed carries no venue field to confirm a specific stadium name.

Team IDs cross-checked against the goal-scorer contestantId (V. Černý,
"2ez9cvam9lp9jyhng3eh3znb4") and the 1-0 final score in
matchDetails/scores/ft, plus HOME_ID matching leg 1's AWAY_ID and vice
versa (same two clubs, venue swapped).
...
DATA_PATH = os.path.join(
    REPO_ROOT, "CZ Events", "CZ 2026-2027", "2026-08-13_Beşiktaş Jimnastik Kulübü - FC Hradec Králové.json",
)
...
HOME_ID = "2ez9cvam9lp9jyhng3eh3znb4"
AWAY_ID = "1v75g4bk8vzrvu0jmaro6lila"
HOME_NAME = "Beşiktaş Jimnastik Kulübü"
AWAY_NAME = "FC Hradec Králové"
COMPETITION = "European Qualifier, Leg 2 of 2 (Aggregate 2-0 Beşiktaş)"
VENUE = "Beşiktaş home ground, Istanbul"
...
def compute_attack_directions(events):
    """Disabled for THIS report, re-verified on this match's own data
    (not assumed from leg 1): average raw pass x per team per period is
    Beşiktaş 46.3/43.4, Hradec 40.1/48.9 -- all four comfortably or
    narrowly below the 50 threshold, so the heuristic would resolve to
    "no flip" for every one of them anyway even if left active. Directly
    checking with the flip off confirms all 34 shots in the match (both
    teams) land in the sensible 76-101m attacking-third range (of a
    105m pitch), none near either side's own goal. This feed's events
    already come oriented per acting contestantId (own goal always at
    x=0, attacking toward x=100), not from a fixed shared-stadium
    coordinate frame -- so no per-half correction is needed here, same
    conclusion as leg 1. Returning {} makes norm_xy()'s
    directions.get(key, 1) fall back to the default (no flip) for every
