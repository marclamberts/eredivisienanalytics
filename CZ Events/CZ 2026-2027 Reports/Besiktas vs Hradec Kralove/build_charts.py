"""
Beşiktaş Jimnastik Kulübü vs FC Hradec Králové - Post-Match Analysis
==========================================================
European qualifier, leg 2 of 2 (2026-08-13, Beşiktaş home ground,
Istanbul). Beşiktaş won 1-0 at home (V. Černý 39') to complete a 2-0
aggregate win over the tie -- Hradec Králové are eliminated. See the
sibling "Hradec Kralove vs Besiktas" report for leg 1 (2026-08-06,
Hradec 0-1 Beşiktaş).

Same template as the sibling "Hradec Kralove vs Besiktas" leg-1 report
(itself carried from "Hradec Kralove vs Pardubice"), re-pointed at this
fixture with home/away swapped, Meridian house style (dark mode) --
housestyle/ package at the repo root. Every team-vs-team page gets a
same-team-both-sides page too, plus heatmaps, zone 14 / half-space
passing, recoveries, duels, discipline, a momentum timeline, and a
simple impact leaderboard.
...
(Model/model_xg.pkl, via Model/xg_model.py) -- same fix already applied
to the leg-1 report. PPDA uses the standard tackles + interceptions +
challenges + fouls-committed action set in the opponent's own 60% of
the pitch -- see match_data.compute_ppda for the Foul outcome-convention
...
FIGSIZE = (13.33, 7.5)
HOME_C = CATEGORICAL_DARK[0]   # ink blue -- Beşiktaş Jimnastik Kulübü (home)
AWAY_C = CATEGORICAL_DARK[1]   # terracotta -- FC Hradec Králové (away)
GOOD_C = STATUS_DARK["good"]
WARN_C = STATUS_DARK["warning"]
HOME_SHORT = "Beşiktaş"
AWAY_SHORT = "Hradec Kr."
...
def match_summary(shots, passes, touches):
    ...
    components.header(fig, kicker="Match Summary",
                       title=f"{md.HOME_NAME} 1-0 {md.AWAY_NAME}",
...
def xg_flow(shots):
    ...
    components.header(fig, kicker="xG Flow",
                       ...
                       dek=f"{md.HOME_NAME} 1-0 {md.AWAY_NAME}  ·  cumulative expected goals by minute",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def goal_buildups(events, directions, shots):
    ...
    components.header(fig, kicker="Goal Build-Ups",
                       ...
                       dek=f"{md.HOME_NAME} 1-0 {md.AWAY_NAME}  ·  possession chain leading to each goal",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def pass_network_combined(passes):
    ...
    components.header(fig, kicker="Pass Network",
                       title="Beşiktaş circulated the ball more, and more accurately",
...
def possession_thirds(touches):
    ...
    components.header(fig, kicker="Possession",
                       title="Both sides spent similar time in the final third -- territory wasn't the story",
...
def progression_bars(passes):
    ...
    components.header(fig, kicker="Progression",
                       title=f"{md.HOME_NAME} edged the progression numbers, and made it count in front of goal",
...
def defensive_actions(defs):
    ...
    components.header(fig, kicker="Defending",
                       title="A similar workload, a different shape: Hradec won it back with interceptions",
...
def ppda(passes, pressing_actions):
    ...
    components.header(fig, kicker="Pressing",
                       title="Hradec pressed hardest exactly when chasing the game late; Beşiktaş simply sat off",
...
def momentum_timeline(shots, cards, subs):
    ...
    components.header(fig, kicker="Match Narrative",
                       ...
                       dek=f"{md.HOME_NAME} 1-0 {md.AWAY_NAME}  ·  key moments timeline",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
...
def win_probability_situation(shots, sim):
    ...
    components.header(fig, kicker="Match Odds",
                       ...
                       dek=f"{sim['n']:,}-simulation Monte Carlo from shot xG  ·  actual result 1-0",
                       palette=palette)
    components.footer(fig, source=md.SOURCE, palette=palette)
